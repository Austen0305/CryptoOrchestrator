...
async def test_batch_price_fetching():
    ...
    try:
        from server_fastapi.services.market_data_service import get_market_data_service
        ...
        print(
            f"    Prices fetched: {len([p for p in sequential_prices.values() if p is not None])}/{len(symbols)}"
        )
        ...
        start_time = time.time()
        batch_prices = await market_data.get_prices_batch(symbols)
        batch_time = time.time() - start_time
        print(f"    Time: {batch_time:.2f}s")
        print(
            f"    Prices fetched: {len([p for p in batch_prices.values() if p is not None])}/{len(symbols)}"
        )
        ...
        large_symbol_list = [
            f"TOKEN{i}/USD" for i in range(60)
        ]  # 60 symbols (will be chunked)

        start_time = time.time()
        large_batch_prices = await market_data.get_prices_batch(large_symbol_list)
        large_batch_time = time.time() - start_time

        print(f"  Fetched {len(large_symbol_list)} symbols in {large_batch_time:.2f}s")
        print(
            f"  Prices returned: {len([p for p in large_batch_prices.values() if p is not None])}"
        )

        # Test 3: Test caching
        print("\n3. Testing caching...")
        cached_start = time.time()
        cached_prices = await market_data.get_prices_batch(
            symbols[:3]
        )  # Should use cache
        ...
        start_time = time.time()
        mixed_prices = await market_data.get_prices_batch(mixed_symbols)
        mixed_time = time.time() - start_time

        print(f"  Mixed fetch time: {mixed_time:.2f}s")
        print(
            f"  Prices returned: {len([p for p in mixed_prices.values() if p is not None])}/{len(mixed_symbols)}"
        )
