...
const App = () => {
  ...
  const handleNotificationNavigation = useCallback((notificationData: unknown) => {
    if (!navigationRef.current) return;

    const { type, trade_id, bot_id } = notificationData || {};
  ...
  const initializePushNotifications = useCallback(async () => {
    try {
      ...
      const receivedListener = Notifications.addNotificationReceivedListener((notification) => {
        console.warn("Notification received (foreground):", notification);
        ...
        Alert.alert(title ?? "Notification", body ?? "You have a new notification", [
          {
            text: "View",
            onPress: () => handleNotificationNavigation(data as Record<string, unknown>),
          },
          {
            text: "Dismiss",
            style: "cancel",
  ...
  const initializeOfflineService = useCallback(() => {
    // Listen to offline service events
    offlineService.on("networkStatusChanged", (isOnline: boolean) => {
      console.warn("Network status changed:", isOnline);
    });

    offlineService.on("syncCompleted", (result: unknown) => {
      console.warn("Sync completed:", result);
    });
  }, []);
  ...
  const initializeApp = useCallback(async () => {
    try {
      ...
      if (biometricResult.available) {
        ...
        const authResult = await BiometricAuth.authenticate("Unlock CryptoOrchestrator");
  ...
  useEffect(() => {
    // Shared DAL Integration Check
    api
      .getPortfolio()
      .then((p) => console.warn("Shared DAL Portfolio:", p))
      .catch((e) => console.warn("Shared DAL (Offline/Mock):", e.message));

    initializeApp();
    initializePushNotifications();
    initializeOfflineService();
  }, [initializeApp, initializePushNotifications, initializeOfflineService]);
    ...
    <QueryClientProvider client={queryClient}>
      <NavigationContainer ref={navigationRef}>
        ...
        >
          ...
          />
          <Tab.Screen name="Trading" component={TradingScreen} options={{ title: "Trading" }} />
