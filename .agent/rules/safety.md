...
# Workspace Rules for CryptoOrchestrator

## Rule 0: AI Safety & HITL
- **No Bypass**: The AI assistant MUST NOT bypass `RiskManager` or `server_fastapi/utils/validation_2026.py` for any financial logic.
- **Human-in-the-Loop**: Trade signing and wallet key management require explicit user validation via `notify_user` before execution.
- **Regulatory Alignment**: All state changes in trading modules MUST align with **ESMA MiCAR RTS (2026)** technical standards for transaction record-keeping (ISO 20022 based JSON schemas).
...
- **Idempotency**: All balance-changing operations MUST use `real_money_transaction_manager.py` with mandatory `idempotency_key` verification.
...
- **Frontend**: Use React 19+ with functional components, **Server Components**, and **Server Actions**. Use `useActionState` for form management. Ensure all components follow the UI Modernization standards for premium aesthetics.
...
- **Recursive Reasoning**: Use `sequential-thinking` for all architectural and safety-critical decisions.
...
- **E2E**: Use 'npm run test:e2e:complete' for critical path verification and chaos simulation.
