...
import { useStartTrailingBot, useStopTrailingBot, useDeleteTrailingBot } from "@/hooks/useApi";
...
interface TrailingBotCardProps {
  bot: any;
}

export function TrailingBotCard({ bot }: TrailingBotCardProps) {
  const startBot = useStartTrailingBot();
  const stopBot = useStopTrailingBot();
  const deleteBot = useDeleteTrailingBot();
    ...
    <Card>
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <CardDescription className="mt-1">
              {bot.symbol} • {bot.bot_type?.replace("_", " ")}
            </CardDescription>
          </div>
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            <p className="text-muted-foreground">Trailing %</p>
            <p className="font-medium">{bot.trailing_percent}%</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="font-medium">${bot.current_price?.toFixed(2) || "0.00"}</p>
          </div>
          <div>
        ...
        <div className="flex gap-2 pt-2">
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>Delete Trailing Bot?</AlertDialogTitle>
                <AlertDialogDescription>
