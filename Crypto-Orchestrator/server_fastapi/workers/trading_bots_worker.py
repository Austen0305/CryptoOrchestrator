            ...
            async with async_session() as session:
                ...
                for bot in active_bots:
                    try:
                        ...
                        results.append(
                            {"bot_id": bot.id, "user_id": bot.user_id, **result}
                        )
                    except Exception as e:
                        logger.error(
                            f"Error processing grid cycle for bot {bot.id}: {e}",
                            exc_info=True,
                        )
                        results.append(
                            {
                ...
                return {
                    "processed": len(
                        [r for r in results if r.get("action") == "processed"]
                    ),
...
def process_infinity_grids():
    ...
    async def run():
        try:
            async with async_session() as session:
                from ..repositories.infinity_grid_repository import (
                    InfinityGridRepository,
                )
                ...
                for bot in active_bots:
                    try:
                        result = await service.process_infinity_grid_cycle(
                            bot.id, bot.user_id
                        )
                        results.append(
                            {"bot_id": bot.id, "user_id": bot.user_id, **result}
                        ...
                        logger.error(
                            f"Error processing infinity grid cycle for bot {bot.id}: {e}",
                            exc_info=True,
                        )
                        results.append(
                            {
    ...
    async def run():
        try:
            async with async_session() as session:
                ...
                for bot in active_bots:
                    try:
                        result = await service.process_trailing_bot_cycle(
                            bot.id, bot.user_id
                        )
                        results.append(
                            {"bot_id": bot.id, "user_id": bot.user_id, **result}
                        ...
                        logger.error(
                            f"Error processing trailing bot cycle for bot {bot.id}: {e}",
                            exc_info=True,
                        )
                        results.append(
                            {
                ...
                return {
                    "processed": len(
                        [r for r in results if r.get("action") == "executed"]
                    ),
                    "monitoring": len(
                        [r for r in results if r.get("action") == "monitoring"]
                    ),
                    "errors": len([r for r in results if r.get("action") == "error"]),
                    "results": results,
                }
...
def update_futures_pnl():
    ...
    async def run():
        try:
            async with async_session() as session:
                from ..repositories.futures_position_repository import (
                ...
                for position in open_positions:
                    try:
                        result = await service.update_position_pnl(
                ...
                return {
                    "updated": len(
                        [r for r in results if r.get("action") == "updated"]
                    ),
                    "closed": len([r for r in results if r.get("action") == "closed"]),
                    "liquidated": len(
                        [r for r in results if r.get("action") == "liquidated"]
                    ),
                    "errors": len([r for r in results if r.get("action") == "error"]),
                    "results": results,
                }
                ...
                for follow in active_follows:
                    try:
                        ...
                        copied_count = await service.auto_copy_recent_trades(
                            follower_id=follow.follower_id, trader_id=follow.trader_id
                        )
                        results.append(
                            {
                                "follow_id": follow.id,
                        ...
                        logger.error(
                            f"Error auto-copying trades for follow {follow.id}: {e}",
                            exc_info=True,
                        )
                        results.append({"follow_id": follow.id, "error": str(e)})

                return {
                    "processed": len(
                        [r for r in results if r.get("copied_trades", 0) > 0]
                    ),
