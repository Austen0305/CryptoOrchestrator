...
crypto_db_connections_active = Gauge(
    "crypto_db_connections_active", "Active database connections"
)

crypto_db_connections_idle = Gauge(
    "crypto_db_connections_idle", "Idle database connections"
)

# Redis Metrics (using unique prefixes)
crypto_redis_connections_active = Gauge(
    "crypto_redis_connections_active", "Active Redis connections"
)

crypto_redis_cache_hits = Counter(
    "crypto_redis_cache_hits_total", "Total Redis cache hits"
)

crypto_redis_cache_misses = Counter(
    "crypto_redis_cache_misses_total", "Total Redis cache misses"
)

# ML Model Metrics (using unique prefixes)
crypto_ml_model_inference_total = Counter(
    "crypto_ml_model_inference_total", "Total ML model inferences", ["model_type"]
)
...
crypto_trading_bots_active = Gauge(
    "crypto_trading_bots_active", "Number of active trading bots"
)

crypto_trades_total = Counter(
    "crypto_trades_total", "Total trades executed", ["exchange", "symbol", "side"]
)

# WebSocket Metrics (using unique prefixes)
crypto_websocket_connections_active = Gauge(
    "crypto_websocket_connections_active", "Active WebSocket connections"
)

crypto_websocket_messages_total = Counter(
    "crypto_websocket_messages_total", "Total WebSocket messages", ["type"]
)

# System Metrics (using unique prefixes)
crypto_system_cpu_percent = Gauge(
    "crypto_system_cpu_percent", "System CPU usage percentage"
)
