...
class TestCoverageHelpers:
    ...
    async def create_test_bot(
        ...
        bot_data = {
            ...
            "symbol": "BTC/USD",
            "strategy": "momentum",
            "config": {"risk_level": "medium", "position_size": 0.1},
        }
...
class TestAdditionalCoverage:
    ...
    async def test_large_payload(
        ...
        response = await client.post(
            "/api/bots/", json=large_data, headers=auth_headers
        )
    ...
    async def test_invalid_token(self, client: AsyncClient):
        ...
        response = await client.get(
            "/api/bots/", headers={"Authorization": "Bearer invalid_token_12345"}
        )
        assert response.status_code == 401
    ...
    async def test_malformed_token(self, client: AsyncClient):
        ...
        response = await client.get(
            "/api/bots/", headers={"Authorization": "Bearer not.a.valid.jwt"}
        )
        assert response.status_code == 401
    ...
    async def test_wrong_auth_scheme(self, client: AsyncClient):
        ...
        response = await client.get(
            "/api/bots/", headers={"Authorization": "Basic dXNlcjpwYXNz"}
        )
