...
from sqlalchemy import Column, Integer, String, Text, Boolean, ForeignKey, JSON, DateTime, Float, Index
...
class UserEvent(Base, TimestampMixin):
    ...
    # Event details
    event_type = Column(String(100), nullable=False, index=True)  # page_view, click, form_submit, etc.
    event_name = Column(String(200), nullable=False, index=True)  # Specific event name
    event_category = Column(String(100), nullable=True, index=True)  # navigation, interaction, conversion, etc.
...
class FeatureUsage(Base, TimestampMixin):
    ...
    __table_args__ = (
        Index("idx_feature_usage_user_feature", "user_id", "feature_name", "created_at"),
...
class ConversionFunnel(Base, TimestampMixin):
    ...
    # Funnel details
    funnel_name = Column(String(100), nullable=False, index=True)  # signup, first_trade, subscription, etc.
    ...
    dropped_at_stage = Column(String(100), nullable=True)  # Stage where user dropped off
...
class UserJourney(Base, TimestampMixin):
    ...
    # Journey details
    journey_type = Column(String(100), nullable=False, index=True)  # onboarding, trading, support, etc.
...
class UserSatisfaction(Base, TimestampMixin):
    ...
    survey_type = Column(String(100), nullable=False, index=True)  # nps, csat, ces, etc.
    ...
    __table_args__ = (
        Index("idx_user_satisfaction_user_type", "user_id", "survey_type", "created_at"),
