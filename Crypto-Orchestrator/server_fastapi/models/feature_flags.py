...
class FeatureFlag(Base, TimestampMixin):
    ...
    # Status and rollout
    status = Column(SQLEnum(FlagStatus), default=FlagStatus.DISABLED, nullable=False, index=True)
    ...
    experiments = relationship("ABTestExperiment", back_populates="feature_flag", cascade="all, delete-orphan")

    __table_args__ = (
        Index("idx_feature_flags_status", "status", "created_at"),
    )
...
class ABTestExperiment(Base, TimestampMixin):
    ...
    # Results
    primary_metric = Column(String(100), nullable=False)  # Conversion rate, revenue, etc.
...
class ExperimentAssignment(Base, TimestampMixin):
    ...
    __table_args__ = (
        Index("idx_experiment_assignments_experiment_user", "experiment_id", "user_id", unique=True),
        Index("idx_experiment_assignments_variant", "variant", "converted", "created_at"),
    )
