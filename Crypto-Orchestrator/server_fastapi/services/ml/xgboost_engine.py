...
class XGBoostEngine:
    ...
    def initialize_model(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"XGBoost model initialized: max_depth={self.config.max_depth}, "
    ...
    def create_features(
        ...
        for i in range(lookback, len(market_data)):
            ...
            if len(prices) >= 14:
                gains = [
                    max(0, prices[j] - prices[j - 1])
                    for j in range(len(prices) - 13, len(prices))
                ]
                losses = [
                    max(0, prices[j - 1] - prices[j])
                    for j in range(len(prices) - 13, len(prices))
                ]
        ...
        try:
            ...
            self.model.fit(
                ...
                early_stopping_rounds=(
                    self.config.early_stopping_rounds if X_val is not None else None
                ),
                verbose=False,
            )
            ...
            return {
                ...
                "history": {
                    ...
                    "val_loss": (
                        results.get("validation_1", {}).get("mlogloss", [])
