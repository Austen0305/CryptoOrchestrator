...
class TransformerEngine:
    ...
    def _transformer_encoder_layer(
        self, d_model: int, num_heads: int, dff: int, dropout: float, name: str
    ):
    ...
    def initialize_model(self) -> None:
        ...
        try:
            # Input layer
            inputs = layers.Input(
                shape=(self.config.sequence_length, self.feature_count)
            )
            ...
            positional_encoding = layers.Embedding(
                input_dim=self.config.sequence_length, output_dim=self.config.d_model
            ...
            self.model = model
            logger.info(
                f"Transformer model initialized: {self.config.num_layers} layers, "
