...
class MarketRegimeService:
    ...
    def detect_regime(
        ...
        try:
            ...
            recent_volumes = (
                volumes[-lookback_period:] if volumes else [1.0] * lookback_period
            )
            ...
            regime, confidence = self._classify_regime(
                trend_strength, volatility, volume_trend, rsi, macd_signal
            )
    ...
    def _calculate_macd_signal(self, prices: List[float]) -> float:
        ...
        if len(macd_line) == len(signal_line):
            ...
            signal_strength = (
                np.tanh(histogram / prices_array[-1]) if prices_array[-1] != 0 else 0.0
            )
