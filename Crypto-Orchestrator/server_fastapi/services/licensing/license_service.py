...
class LicenseService:
    ...
    def generate_license_key(
        ...
        try:
            ...
            config = self.LICENSE_CONFIG.get(
                license_type, self.LICENSE_CONFIG[LicenseType.TRIAL]
            )
            ...
            # Create signature
            signature = hmac.new(
                self.secret_key.encode(), data_encoded.encode(), hashlib.sha256
            ).hexdigest()
            ...
            logger.info(
                f"Generated license key for user {user_id}, type {license_type}"
            )
    ...
    def validate_license_key(
        ...
        try:
            ...
            return LicenseStatus(
                valid=False, license_type="error", message=f"Validation error: {str(e)}"
            )
