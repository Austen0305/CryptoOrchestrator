...
class PaperTradingService:
    ...
    async def execute_paper_trade(
        self, user_id: str, symbol: str, side: str, quantity: float, price: float
    ) -> PaperTrade:
        ...
        if side == "buy":
            ...
            if portfolio.available_balance < total_cost:
                raise ValueError(
                    f"Insufficient balance. Required: {total_cost}, Available: {portfolio.available_balance}"
                )
            ...
            if not position or position.quantity < quantity:
                raise ValueError(
                    f"Insufficient position for {symbol}. Available: {position.quantity if position else 0}"
                )
            ...
            pnl = (
                total_value - cost_basis - (total_value * 0.001)
            )  # Subtract commission
            ...
            new_balance = (
                portfolio.available_balance + total_value - (total_value * 0.001)
            )
            await self._update_portfolio_balance(user_id, new_balance, new_balance)
        ...
        logger.info(
            f"Paper trade executed: {trade.id} - {side} {quantity} {symbol} at {price}"
        )
        return trade

    @(
        cache_query_result(ttl=300, key_prefix="paper_portfolio", include_user=True)
        if CACHE_AVAILABLE
        else lambda f: f
    )
    async def get_paper_portfolio(self, user_id: str) -> PaperPortfolio:
        ...
        total_pnl_percentage = (
            (total_pnl / (total_balance - total_pnl)) * 100
            if total_balance > total_pnl
            else 0
        )
    ...
    async def _update_portfolio_balance(
        self, user_id: str, total_balance: float, available_balance: float
    ):
