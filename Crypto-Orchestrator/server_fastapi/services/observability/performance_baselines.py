...
class PerformanceBaselinesService:
    ...
    def calculate_baseline(
        ...
        percentile_95 = sorted_values[int(len(sorted_values) * 0.95)] if len(sorted_values) > 1 else sorted_values[-1]
        percentile_99 = sorted_values[int(len(sorted_values) * 0.99)] if len(sorted_values) > 1 else sorted_values[-1]
    ...
    def analyze_trend(
        ...
        times = [(t - recent_data[0][0]).total_seconds() / 3600.0 for t, _ in recent_data]
    ...
    def get_trend(self, metric_name: str) -> Optional[Trend]:
