...
class AlertEscalationService:
    ...
    def escalate_alert(
        ...
        if policy.steps:
            ...
            escalation.next_escalation = (
                datetime.utcnow() + timedelta(minutes=first_step.delay_minutes)
            )
    ...
    def _send_notification(
        ...
        logger.info(f"Sending notification to {target} for alert {escalation.alert_key}")
            ...
            if escalation.next_escalation and now >= escalation.next_escalation:
                ...
                if policy:
                    ...
                    if escalation.current_step < len(policy.steps):
                        ...
                        escalation.next_escalation = (
                            now + timedelta(minutes=next_step.delay_minutes)
                        )
                    else:
                        # All steps completed
                        escalation.completed = True
    ...
    def get_on_call_rotation(self, rotation_id: str) -> Optional[OnCallRotation]:
