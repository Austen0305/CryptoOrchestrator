...
class LogSanitizer:
    ...
    def sanitize_dict(
        ...
        for key, value in data.items():
            ...
            if any(
                sensitive in key_lower for sensitive in LogSanitizer.SENSITIVE_FIELDS
            ):
                sanitized[key] = mask
    ...
    def sanitize_list(data: List[Any], mask: str = "***MASKED***") -> List[Any]:
        ...
        return [
            (
                LogSanitizer.sanitize_dict(item, mask)
                if isinstance(item, dict)
                else (
                    LogSanitizer.sanitize_list(item, mask)
                    if isinstance(item, list)
                    else (
                        LogSanitizer.sanitize_string(item, mask)
