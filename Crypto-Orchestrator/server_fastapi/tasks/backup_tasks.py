...
def create_daily_backup():
    ...
    try:
        ...
        if result.get("status") == "success":
            logger.info(f"[OK] Daily backup completed: {result.get('filename')}")
        else:
            logger.error(f"[ERROR] Daily backup failed: {result.get('error')}")
...
def cleanup_old_backups():
    ...
    try:
        ...
        deleted_count = loop.run_until_complete(backup_service._cleanup_old_backups())

        logger.info(f"[OK] Cleaned up {deleted_count} old backups")
