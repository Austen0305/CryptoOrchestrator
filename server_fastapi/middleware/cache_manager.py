...
def cached(ttl: int = 300, prefix: str = "", key_builder: Callable | None = None):
    ...
    def decorator(func: Callable):
        ...
        async def wrapper(*args, **kwargs):
            ...
            full_key = (
                f"{prefix}:{func.__name__}:{key}"
                if prefix
                else f"{func.__name__}:{key}"
            )
...
class CacheWarmer:
    ...
    async def warm_cache(
        self, func: Callable, args: tuple, kwargs: dict, ttl: int, prefix: str = ""
    ):
        """
        ...
        full_key = (
            f"{prefix}:{func.__name__}:{cache_key_val}"
            if prefix
            else f"{func.__name__}:{cache_key_val}"
        )
    ...
    def start_warming(
        ...
        task = asyncio.create_task(self.warm_cache(func, args, kwargs, ttl, prefix))
