...
class DatabasePoolMonitor:
    ...
    def record_metrics(
        ...
        metric = PoolMetrics(
            ...
            checkout_time_ms=sum(self.checkout_times) / len(self.checkout_times)
    ...
    def get_stats(self, window_minutes: int = 5) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_active": sum(m.active_connections for m in recent_metrics)
            / len(recent_metrics),
            "max_active": max(m.active_connections for m in recent_metrics),
            "avg_idle": sum(m.idle_connections for m in recent_metrics)
            / len(recent_metrics),
            "avg_checkout_time_ms": sum(m.checkout_time_ms for m in recent_metrics)
            / len(recent_metrics),
