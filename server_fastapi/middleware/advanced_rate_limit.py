...
class AdvancedRateLimiter:
    ...
    async def check_rate_limit(
        ...
        if not limit_config:
            limit_config = self.default_limits.get(user_tier, self.default_limits["anonymous"])
        ...
        if not self.redis_available:
            return self._check_memory_limit(identifier, endpoint, requests_limit, window_seconds)
            ...
            raise HTTPException(
                ...
                detail={
                    ...
                    "limit": self.rate_limiter.endpoint_limits.get(
                        endpoint, 
                        self.rate_limiter.default_limits[user_tier]
                    )["requests"]
                },
                headers={
                    "X-RateLimit-Limit": str(self.rate_limiter.endpoint_limits.get(
                        endpoint,
                        self.rate_limiter.default_limits[user_tier]
                    )["requests"]),
