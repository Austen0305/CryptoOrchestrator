...
class TradingValidators:
    ...
    @staticmethod
    def validate_amount(amount: float, min_amount: float = 0.0, max_amount: Optional[float] = None, field_name: str = "Amount") -> float:
        """
    ...
    @staticmethod
    def validate_price(price: float, min_price: float = 0.0, field_name: str = "Price") -> float:
        """
    ...
    @staticmethod
    def validate_balance_sufficient(current_balance: float, required_amount: float) -> None:
        """
...
def validate_trade_params(symbol: str, amount: float, price: Optional[float] = None) -> tuple:
    """
    ...
    validated_amount = TradingValidators.validate_amount(amount, min_amount=0.0, field_name="Trade amount")
    validated_price = TradingValidators.validate_price(price, field_name="Price") if price else None
...
def validate_bot_config(name: str, symbol: str, amount: float) -> dict:
    ...
    validated_symbol = TradingValidators.validate_symbol(symbol)
    validated_amount = TradingValidators.validate_amount(amount, min_amount=1.0, field_name="Bot amount")
