...
SQL_INJECTION_PATTERNS = [
    re.compile(r"(?i)(union|select|insert|update|delete|drop|create|alter|exec|execute)"),
    re.compile(r"(?i)(--|/\*|\*/|;|'|%)"),
]
XSS_PATTERNS = [
...
def validate_hex_string(value: str, min_length: int = 0, max_length: Optional[int] = None) -> str:
    """
...
def validate_amount(amount: float, min_value: float = 0.0, max_value: Optional[float] = None) -> float:
    """
...
def validate_pagination(page: int = 1, per_page: int = 20, max_per_page: int = 100) -> tuple[int, int]:
    """
