...
class Settings(BaseSettings):
    ...
    exchange_key_encryption_key: str = Field(
        default="dev-key-32-bytes-long-change-me",
        alias="EXCHANGE_KEY_ENCRYPTION_KEY"
    )
    ...
    # Rate Limiting
    enable_distributed_rate_limit: bool = Field(default=False, alias="ENABLE_DISTRIBUTED_RATE_LIMIT")
    ...
    # Cold Storage
    cold_storage_threshold: float = Field(default=10000.0, alias="COLD_STORAGE_THRESHOLD")
    cold_storage_processing_hours: int = Field(default=24, alias="COLD_STORAGE_PROCESSING_HOURS")
    
    # Staking
    staking_min_amount: float = Field(default=10.0, alias="STAKING_MIN_AMOUNT")
    staking_reward_distribution_time: str = Field(default="00:00", alias="STAKING_REWARD_DISTRIBUTION_TIME")
    ...
    def validate_jwt_secret(cls, v, info):
        ...
        production_mode = info.data.get("production_mode", False) or info.data.get("node_env") == "production"
        if production_mode and (not v or v in ["change-me-in-production-use-strong-random-secret", "dev-secret-change-me-in-production"]):
            raise ValueError("JWT_SECRET must be set to a strong random secret in production")
        return v
    ...
    def validate_encryption_key(cls, v, info):
        ...
        production_mode = info.data.get("production_mode", False) or info.data.get("node_env") == "production"
        if production_mode and (not v or v in ["change-me-in-production-use-32-byte-key", "dev-key-32-bytes-long-change-me"]):
            raise ValueError("EXCHANGE_KEY_ENCRYPTION_KEY must be set to a 32-byte key in production")
        return v
    ...
    def validate_node_env(cls, v):
        if v not in ["development", "staging", "production", "test"]:
            raise ValueError("NODE_ENV must be one of: development, staging, production, test")
        return v
    ...
    def validate_mock_data_in_production(cls, v, info):
        ...
        if production_mode and v:
            raise ValueError("Cannot enable mock data in production mode. Set ENABLE_MOCK_DATA=false and PRODUCTION_MODE=true")
        return v
