            ...
            async with get_db_context() as db:
                ...
                for key_obj in keys:
                    try:
                        ...
                        api_secret = self._decrypt_with_old_key(key_obj.api_secret_encrypted)
                        passphrase = None
                        if key_obj.passphrase_encrypted:
                            passphrase = self._decrypt_with_old_key(key_obj.passphrase_encrypted)
                        
                        # Encrypt with new key
                        key_obj.api_key_encrypted = self._encrypt_with_new_key(api_key)
                        key_obj.api_secret_encrypted = self._encrypt_with_new_key(api_secret)
                        if passphrase:
                            key_obj.passphrase_encrypted = self._encrypt_with_new_key(passphrase)
                        
                        stats["rotated"] += 1
                        logger.info(f"Rotated key {key_obj.id} for user {key_obj.user_id} on {key_obj.exchange}")
...
async def main():
    ...
    if args.generate_key:
        ...
        print(f"\n⚠️  Keep this key secure! Store it in your environment variables.")
        return
    ...
    if not old_key:
        logger.error("Old encryption key not provided. Use --old-key or set EXCHANGE_KEY_ENCRYPTION_KEY_OLD")
        sys.exit(1)
    ...
    if not new_key:
        ...
        print(f"⚠️  Save this key! Set it as EXCHANGE_KEY_ENCRYPTION_KEY in your environment.")
    ...
    try:
        ...
        if args.dry_run:
            ...
            print("\n✅ Rotation complete!")
            print(f"\n⚠️  Don't forget to update EXCHANGE_KEY_ENCRYPTION_KEY={new_key}")
