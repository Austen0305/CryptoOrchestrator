...
class TestTradingWorkflow:
    ...
    async def test_create_and_start_bot_workflow(
        ...
        create_response = await client.post(
            "/api/bots/", json=bot_data, headers=auth_headers
        )
        assert create_response.status_code == 200
        created_bot = create_response.json()
        bot_id = created_bot["id"]
        assert created_bot["name"] == bot_data["name"]
        # Bot starts as inactive - check is_active or status
        assert created_bot.get("is_active") is False or created_bot.get("status") in [
            "inactive",
            "stopped",
        ]
        ...
        # Note: Cache may return stale data, so check both is_active and status
        status_response = await client.get(f"/api/bots/{bot_id}", headers=auth_headers)
        assert status_response.status_code == 200
        bot_status = status_response.json()
        # Bot should be active after start - check is_active (boolean) or status (string)
        is_active = bot_status.get("is_active", False)
        status = bot_status.get("status", "stopped")
        assert is_active is True or status in [
            "active",
            "running",
            "started",
        ], f"Bot not active: is_active={is_active}, status={status}"
    ...
    async def test_paper_trading_workflow(
        ...
        trade_response = await client.post(
            "/api/trades/", json=trade_data, headers=auth_headers
        )
        ...
        trades_response = await client.get(
            f"/api/trades/?botId={bot_id}", headers=auth_headers
        )
    ...
    async def test_bot_update_and_stop_workflow(
        ...
        assert bot_status.get("is_active") is False or bot_status.get("status") in [
            "stopped",
            "inactive",
        ]
    ...
    async def test_portfolio_tracking_workflow(
        ...
        portfolio_response = await client.get(
            "/api/portfolio/paper", headers=auth_headers
        )
    ...
    async def test_risk_management_integration(
        ...
        risk_response = await client.get("/api/risk/metrics", headers=auth_headers)
        ...
        if markets:
            ...
            if market_id:
                market_response = await client.get(
                    f"/api/markets/{market_id}", headers=auth_headers
                )
...
class TestErrorHandling:
    ...
    async def test_invalid_trade_creation(
        ...
        response = await client.post(
            "/api/trades/", json=invalid_trade, headers=auth_headers
        )
        assert response.status_code in [400, 422]
    ...
    async def test_nonexistent_bot_operations(
        ...
        response = await client.post(
            f"/api/bots/{fake_bot_id}/start", headers=auth_headers
        )
        assert response.status_code == 404
