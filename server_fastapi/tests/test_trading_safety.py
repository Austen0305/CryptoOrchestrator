...
class TestTradeValidation:
    ...
    def test_daily_loss_limit_triggers_kill_switch(
        self, safety_service, mock_account_state
    ):
        """Test that daily loss limit triggers kill switch."""
        # Record losses to hit daily limit
        safety_service.record_trade_result(
            "trade1", -300.0, "BTC/USDT", "buy", 0.01, 50000.0
        )
        safety_service.record_trade_result(
            "trade2", -250.0, "BTC/USDT", "sell", 0.01, 49000.0
        )
    ...
    def test_consecutive_losses_trigger_kill_switch(
        ...
        safety_service.record_trade_result(
            "trade1", -100.0, "BTC/USDT", "buy", 0.01, 50000.0
        )
        safety_service.record_trade_result(
            "trade2", -100.0, "ETH/USDT", "buy", 0.1, 3000.0
        )
        safety_service.record_trade_result(
            "trade3", -100.0, "BTC/USDT", "sell", 0.01, 49000.0
        )
...
class TestTradeRecording:
    ...
    def test_profit_resets_consecutive_losses(self, safety_service):
        ...
        # Record a profit
        safety_service.record_trade_result(
            "trade3", 150.0, "BTC/USDT", "sell", 0.01, 51500.0
        )
    ...
    def test_daily_pnl_accumulates(self, safety_service):
        ...
        safety_service.record_trade_result(
            "trade2", -50.0, "ETH/USDT", "buy", 0.1, 3000.0
        )
        safety_service.record_trade_result(
            "trade3", 75.0, "BTC/USDT", "sell", 0.01, 50750.0
        )
