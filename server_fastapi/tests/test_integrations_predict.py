...
def auth_headers(client):
    ...
    reg = client.post(
        "/api/auth/register",
        json={"email": email, "password": "PredictPass123!", "name": "Predict User"},
    )
    assert reg.status_code == 200, reg.text
    login = client.post(
        "/api/auth/login", json={"email": email, "password": "PredictPass123!"}
    )
...
class TestIntegrationsPredict:
    ...
    def test_predict_happy_path(self, client, auth_headers):
        payload = {"symbol": "BTC/USDT", "timeframe": "1h", "data": _sample_ohlcv()}
        res = client.post(
            "/api/integrations/predict", json=payload, headers=auth_headers
        )
