...
class TestPerformanceMetrics:
    ...
    def test_sharpe_ratio_calculation(self):
        ...
        expected_sharpe = (
            (excess_return / std_return * (TRADING_DAYS_PER_YEAR**0.5))
            if std_return > 0
            else 0
        )
    ...
    def test_edge_case_zero_trades(self):
        ...
        win_rate = (
            0
            if not profits
            else (len([p for p in profits if p > 0]) / len(profits) * 100)
        )
...
class TestPerformanceAPIModels:
    ...
    def test_daily_pnl_model(self):
        ...
        pnl = DailyPnL(date="2025-12-01", pnl=150.50, cumulativePnl=1500.00, trades=5)
