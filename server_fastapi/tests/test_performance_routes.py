...
class TestPerformanceRoutes:
    ...
    async def test_get_performance_summary_with_mode(
        ...
        response = await client.get("/api/performance/summary?mode=paper", headers=auth_headers)
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_get_performance_summary_live_mode(
        ...
        response = await client.get("/api/performance/summary?mode=live", headers=auth_headers)
        assert response.status_code == 200
        data = response.json()
