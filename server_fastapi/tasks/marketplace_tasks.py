...
from ..models.signal_provider import CuratorStatus, Payout, SignalProvider
        ...
        try:
            ...
            async with get_db_context() as session:
                ...
                for signal_provider in signal_providers:
                    try:
                        await marketplace_service.update_performance_metrics(
                            signal_provider.id
                        )
                        updated_count += 1
                        logger.debug(
                            f"Updated metrics for signal provider {signal_provider.id}"
                        )
            ...
            logger.error(
                f"Error in update_all_provider_metrics_task: {e}", exc_info=True
            )
            raise
...
def check_underperforming_providers_task(self) -> dict:
    ...
    async def check_providers():
        try:
                ...
                for signal_provider in signal_providers:
                    try:
                        ...
                        if signal_provider.win_rate < MIN_WIN_RATE:
                            ...
                            reasons.append(
                                f"Win rate {signal_provider.win_rate:.1%} below {MIN_WIN_RATE:.1%}"
                            )

                        # Check max drawdown
                        if (
                            signal_provider.max_drawdown > MAX_DRAWDOWN * 100
                        ):  # Convert to percentage
                            is_underperforming = True
                            reasons.append(
                                f"Max drawdown {signal_provider.max_drawdown:.1f}% above {MAX_DRAWDOWN * 100:.1f}%"
                            )
                        ...
                        if signal_provider.sharpe_ratio < MIN_SHARPE_RATIO:
                            is_underperforming = True
                            reasons.append(
                                f"Sharpe ratio {signal_provider.sharpe_ratio:.2f} below {MIN_SHARPE_RATIO}"
                            )
                        ...
                        if is_underperforming:
                            ...
                            try:
                                ...
                                if user and user.email:
                                    ...
                                    await (
                                        email_service.send_underperforming_alert_email(
                                            to_email=user.email,
                                ...
                                logger.error(
                                    f"Failed to send underperforming alert email: {e}",
                                    exc_info=True,
                                )
            ...
            logger.error(
                f"Error in check_underperforming_providers_task: {e}", exc_info=True
            )
            raise
...
def update_single_provider_metrics_task(self, signal_provider_id: int) -> dict:
    ...
    async def update_metrics():
        try:
            ...
            async with get_db_context() as session:
                marketplace_service = MarketplaceService(session)
                result = await marketplace_service.update_performance_metrics(
                    signal_provider_id
                )

                logger.info(
                    f"Metrics updated for signal provider {signal_provider_id}: {result}"
                )
...
def check_analytics_thresholds_task(self) -> dict:
    ...
    async def check_thresholds():
        try:
            ...
            async with get_db_context() as session:
                from ..services.marketplace_threshold_service import (
            ...
            logger.error(
                f"Error in check_analytics_thresholds_task: {e}", exc_info=True
            )
            raise
