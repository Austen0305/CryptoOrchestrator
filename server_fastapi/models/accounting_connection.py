...
class AccountingConnection(Base, TimestampMixin):
    ...
    account_mappings: Mapped[Optional[dict]] = mapped_column(
        JSON, nullable=True
    ...
    __table_args__ = (
        Index("idx_user_system", "user_id", "system", unique=True),
    )
...
class AccountingSyncLog(Base, TimestampMixin):
    ...
    transactions_failed: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False
    )
    ...
    duration_seconds: Mapped[Optional[float]] = mapped_column(
        JSON, nullable=True  # Using JSON to store float (SQLite compatibility)
    )
