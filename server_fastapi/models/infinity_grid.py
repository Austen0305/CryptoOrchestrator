...
class InfinityGrid(BaseModel):
    ...
    grid_spacing_percent: Mapped[float] = mapped_column(Float, nullable=False)  # Spacing between grids (%)
    order_amount: Mapped[float] = mapped_column(Float, nullable=False)  # Amount per order
    
    # Dynamic bounds
    current_upper_price: Mapped[float] = mapped_column(Float, nullable=False)  # Current upper bound
    current_lower_price: Mapped[float] = mapped_column(Float, nullable=False)  # Current lower bound
    initial_price: Mapped[float] = mapped_column(Float, nullable=False)  # Price when bot started
    
    # Adjustment parameters
    upper_adjustment_percent: Mapped[float] = mapped_column(Float, default=5.0, nullable=False)  # Adjust upper when price moves this % up
    lower_adjustment_percent: Mapped[float] = mapped_column(Float, default=5.0, nullable=False)  # Adjust lower when price moves this % down
    ...
    grid_adjustments: Mapped[int] = mapped_column(Integer, default=0, nullable=False)  # Number of times grid was adjusted
    ...
    last_adjustment_at: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)
    ...
    trades: Mapped[list["Trade"]] = relationship("Trade", back_populates="infinity_grid", cascade="all, delete-orphan")
