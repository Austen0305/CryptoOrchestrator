...
class Wallet(Base, TimestampMixin):
    ...
    # Wallet details
    wallet_type: Mapped[str] = mapped_column(String(20), default="trading", nullable=False, index=True)
    currency: Mapped[str] = mapped_column(String(10), default="USD", nullable=False, index=True)  # USD, BTC, ETH, etc.
    
    # Balance
    balance: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)
    available_balance: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)  # Available for trading
    locked_balance: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)  # Locked in orders
...
class WalletTransaction(Base, TimestampMixin):
    ...
    # References
    reference_id: Mapped[Optional[str]] = mapped_column(String(100), nullable=True, index=True)  # External transaction ID
    trade_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("trades.id"), nullable=True, index=True)
    payment_intent_id: Mapped[Optional[str]] = mapped_column(String(100), nullable=True, index=True)  # Stripe payment intent
    
    # Metadata
    description: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    transaction_metadata: Mapped[Optional[dict]] = mapped_column(Text, nullable=True)  # JSON metadata
