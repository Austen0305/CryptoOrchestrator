...
wallet_signer_association = Table(
    ...
    Base.metadata,
    Column(
        "wallet_id", Integer, ForeignKey("institutional_wallets.id"), primary_key=True
    ),
...
class InstitutionalWallet(Base, TimestampMixin):
    ...
    multisig_type: Mapped[str | None] = mapped_column(
    ...
    lock_duration_days: Mapped[int | None] = mapped_column(
    ...
    pending_transactions: Mapped[list["PendingTransaction"]] = relationship(
        "PendingTransaction", back_populates="wallet", cascade="all, delete-orphan"
    )
    guardians: Mapped[list["SocialRecoveryGuardian"]] = relationship(
        "SocialRecoveryGuardian", back_populates="wallet", cascade="all, delete-orphan"
    )
    recovery_requests: Mapped[list["RecoveryRequest"]] = relationship(
        "RecoveryRequest", back_populates="wallet", cascade="all, delete-orphan"
    )
...
class PendingTransaction(Base, TimestampMixin):
    ...
    required_signatures: Mapped[int] = mapped_column(Integer, nullable=False)
...
class InstitutionalWalletTransaction(Base, TimestampMixin):
    ...
    pending_transaction_id: Mapped[int | None] = mapped_column(
    ...
    gas_price: Mapped[int | None] = mapped_column(Integer, nullable=True)
