...
class GridBot(BaseModel):
    ...
    # Grid parameters
    upper_price: Mapped[float] = mapped_column(
        Float, nullable=False
    )  # Upper bound of grid
    lower_price: Mapped[float] = mapped_column(
        Float, nullable=False
    )  # Lower bound of grid
    grid_count: Mapped[int] = mapped_column(
        Integer, nullable=False
    )  # Number of grid levels
    grid_spacing_type: Mapped[str] = mapped_column(
        String(20), default="arithmetic", nullable=False
    ...
    trades: Mapped[list["Trade"]] = relationship(
        "Trade", back_populates="grid_bot", cascade="all, delete-orphan"
    )
