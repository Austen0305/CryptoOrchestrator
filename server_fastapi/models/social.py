...
class SharedStrategy(Base, TimestampMixin):
    ...
    visibility = Column(
        SQLEnum(StrategyVisibility), default=StrategyVisibility.PUBLIC, nullable=False
    )
    share_token = Column(
    ...
    likes = relationship(
        "StrategyLike", back_populates="strategy", cascade="all, delete-orphan"
    )
    comments = relationship(
        "StrategyComment", back_populates="strategy", cascade="all, delete-orphan"
    )
...
class StrategyLike(Base, TimestampMixin):
    ...
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False, index=True)
    strategy_id = Column(
        Integer, ForeignKey("shared_strategies.id"), nullable=False, index=True
    )
...
class StrategyComment(Base, TimestampMixin):
    ...
    parent_comment_id = Column(
        Integer, ForeignKey("strategy_comments.id"), nullable=True
    )  # For replies
    ...
    parent_comment = relationship(
        "StrategyComment", remote_side=[id], backref="replies"
    )
...
class SocialFeedEvent(Base, TimestampMixin):
    ...
    )  # trade_executed, strategy_shared, achievement_earned, etc.
...
class UserProfile(Base, TimestampMixin):
    ...
    __table_args__ = (Index("idx_user_profiles_public", "is_public", "created_at"),)
...
class Achievement(Base, TimestampMixin):
    ...
    )  # trading, social, milestone, etc.

    # Requirements
    requirement_type = Column(
        String(100), nullable=False
    )  # trade_count, profit_amount, win_streak, etc.
    requirement_value = Column(JSON, nullable=False)  # Requirement parameters

    # Metadata
    rarity = Column(
        String(50), default="common", index=True
    )  # common, rare, epic, legendary
    points = Column(Integer, default=0)  # Points awarded

    __table_args__ = (Index("idx_achievements_category", "category", "rarity"),)
...
class CommunityChallenge(Base, TimestampMixin):
    ...
    challenge_type = Column(
        String(100), nullable=False, index=True
    )  # profit_contest, win_rate_contest, etc.
...
class ChallengeParticipant(Base, TimestampMixin):
    ...
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False, index=True)
    challenge_id = Column(
        Integer, ForeignKey("community_challenges.id"), nullable=False, index=True
    )
    ...
    __table_args__ = (
        ...
        Index(
            "idx_challenge_participants_user", "user_id", "challenge_id", unique=True
        ),
    )
