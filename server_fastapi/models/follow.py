...
class Follow(Base, TimestampMixin):
    ...
    trader_id: Mapped[int] = mapped_column(Integer, ForeignKey("users.id"), nullable=False, index=True)
    ...
    # Auto-copy settings
    auto_copy_enabled: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    copy_buy_orders: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    copy_sell_orders: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
...
class CopiedTrade(Base, TimestampMixin):
    ...
    follower_id: Mapped[int] = mapped_column(Integer, ForeignKey("users.id"), nullable=False, index=True)
    trader_id: Mapped[int] = mapped_column(Integer, ForeignKey("users.id"), nullable=False, index=True)
    original_trade_id: Mapped[int] = mapped_column(Integer, ForeignKey("trades.id"), nullable=False, index=True)
    copied_trade_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("trades.id"), nullable=True, index=True)
    ...
    follower: Mapped["User"] = relationship("User", foreign_keys=[follower_id])
    trader: Mapped["User"] = relationship("User", foreign_keys=[trader_id])
    original_trade: Mapped["Trade"] = relationship("Trade", foreign_keys=[original_trade_id])
    copied_trade: Mapped[Optional["Trade"]] = relationship("Trade", foreign_keys=[copied_trade_id])
