...
class TreasuryService:
    ...
    def get_treasury_summary(self, user_id: int) -> TreasurySummary:
        ...
        """
        from ..models.institutional import InstitutionalWallet, PendingTransaction, InstitutionalWalletTransaction
        
        with self.db_session() as session:
            # Get all wallets for user
            wallets = session.query(InstitutionalWallet).filter(
                InstitutionalWallet.owner_id == user_id
            ).all()
            ...
            pending_count = session.query(PendingTransaction).filter(
                PendingTransaction.wallet_id.in_([w.id for w in wallets])
            ).count()
            
            # Get 24h transactions
            cutoff = datetime.utcnow() - timedelta(hours=24)
            transactions_24h = session.query(InstitutionalWalletTransaction).filter(
                InstitutionalWalletTransaction.wallet_id.in_([w.id for w in wallets]),
            ...
            avg_tx_size = (
                ...
                if transactions_24h else Decimal("0")
            )
    ...
    def get_wallet_balances(self, user_id: int) -> List[WalletBalance]:
        ...
        with self.db_session() as session:
            ...
            for wallet in wallets:
                # Get pending transactions
                pending_txs = session.query(PendingTransaction).filter(
    ...
    def get_treasury_activity(
        ...
        with self.db_session() as session:
            ...
            transactions = tx_query.order_by(
                InstitutionalWalletTransaction.created_at.desc()
            ).limit(limit).all()
            
            for tx in transactions:
                wallet = next((w for w in wallets if w.id == tx.wallet_id), None)
                activities.append(TreasuryActivity(
                    timestamp=tx.created_at,
                    wallet_id=tx.wallet_id,
                    wallet_name=wallet.name if wallet else f"Wallet {tx.wallet_id}",
                    activity_type="transfer" if tx.transaction_type == "transfer" else tx.transaction_type or "unknown",
            ...
            access_logs = log_query.order_by(
                WalletAccessLog.timestamp.desc()
            ).limit(limit).all()
            ...
            for log in access_logs:
                ...
                activities.append(TreasuryActivity(
                    ...
                    wallet_name=wallet.name if wallet else f"Wallet {log.wallet_id}",
    ...
    def get_risk_metrics(self, user_id: int) -> Dict[str, Any]:
        ...
        pending_ratio = (
            ...
            if summary.total_balance_usd > 0 else 0
        )
        ...
        return {
            ...
            "risk_level": (
                "high" if pending_ratio > 50 or concentration_ratio > 80
                else "medium" if pending_ratio > 25 or concentration_ratio > 60
                else "low"
            ),
        }
