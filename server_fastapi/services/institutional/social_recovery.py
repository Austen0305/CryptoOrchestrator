...
class SocialRecoveryService:
    ...
    async def add_guardian(
        ...
        stmt = select(SocialRecoveryGuardian).where(
            and_(
                ...
                or_(
                    SocialRecoveryGuardian.guardian_user_id == guardian_user_id if guardian_user_id else False,
    ...
    async def create_recovery_request(
        ...
        recovery_request = RecoveryRequest(
            ...
            expires_at=now + timedelta(hours=72),  # Request expires in 3 days if not approved
        )
    ...
    async def approve_recovery(
        ...
        stmt = select(RecoveryRequest).where(
        ...
        # Check if expired
        if recovery_request.expires_at and datetime.utcnow() > recovery_request.expires_at:
    ...
    async def execute_recovery(
        ...
        if recovery_request.status != RecoveryRequestStatus.APPROVED.value:
            raise ValueError(f"Recovery request not approved (status: {recovery_request.status})")
        ...
        if recovery_request.unlock_time and datetime.utcnow() < recovery_request.unlock_time:
            raise ValueError(
    ...
    async def get_wallet_recovery_requests(
        ...
        stmt = select(RecoveryRequest).where(
            RecoveryRequest.wallet_id == wallet_id
        )
    ...
    async def get_recovery_approvals(
        ...
        stmt = select(RecoveryApproval).where(
