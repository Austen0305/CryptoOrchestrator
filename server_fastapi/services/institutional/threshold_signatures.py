...
try:
    ...
    TSS_AVAILABLE = False
    logger.warning(
        "TSS library not available. Install with: pip install tss-lib or similar"
    )
...
class ThresholdSignatureService:
    ...
    def __init__(self):
        ...
        self.partial_signatures: dict[
            str, list[TSSPartialSignature]
        ] = {}  # message_hash -> partials
    ...
    def generate_partial_signature(
        ...
        if not party_share:
            raise ValueError(
                f"Party {party_id} does not have a key share for wallet {wallet_id}"
            )
        ...
        logger.debug(
            f"Generated partial signature from party {party_id} for wallet {wallet_id}"
        )
    ...
    def combine_signatures(
        ...
        partials = [
            ps
            for ps in self.partial_signatures.get(message_hash, [])
    ...
    def get_signature(self, signature_id: str) -> TSSSignature | None:
