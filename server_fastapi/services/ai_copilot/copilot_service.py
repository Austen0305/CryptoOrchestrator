...
class StrategyOptimizationRequest(BaseModel):
    ...
    optimization_goals: List[
        str
    ]  # e.g., ["maximize_sharpe", "minimize_drawdown", "maximize_returns"]
...
class OptimizationResult(BaseModel):
    ...
    improvements: Dict[
        str, float
    ]  # e.g., {"sharpe_ratio": 0.15, "max_drawdown": -0.05}
    recommendations: List[str]
    confidence_score: float
    timestamp: datetime = Field(default_factory=datetime.utcnow)
...
class BacktestSummaryRequest(BaseModel):
    ...
    focus_areas: Optional[List[str]] = (
        None  # e.g., ["risk", "performance", "optimization"]
    )
...
class AICopilotService:
    ...
    def _generate_reasoning(
        self, trade_data: Dict[str, Any], market_data: Optional[Dict[str, Any]]
    ) -> str:
        ...
        if "support_resistance" in indicators:
            ...
            if "support" in levels and price <= levels["support"] * 1.02:
                reasoning_parts.append(
                    f"• Price near support level at ${levels['support']:.2f}"
                )
            elif "resistance" in levels and price >= levels["resistance"] * 0.98:
                reasoning_parts.append(
                    f"• Price near resistance level at ${levels['resistance']:.2f}"
                )
    ...
    def _calculate_confidence(
        ...
        if market_data:
            ...
            if (
                ...
                and indicators["volume"].get("trend") == "increasing"
            ):
                base_confidence += 0.1
    ...
    def _generate_recommendations(
        ...
        if market_data:
            ...
            if volatility > 0.05:
                recommendations.append(
                    "High volatility detected - consider wider stop-loss"
                )
    ...
    async def generate_strategy_from_text(
        ...
        try:
            ...
            code = self._generate_strategy_code(
                name, entry_conditions, exit_conditions, risk_parameters
            )
    ...
    def _generate_entry_conditions(
        self, request: StrategyGenerationRequest, strategy_type: str
    ) -> Dict[str, Any]:
        """Generate entry conditions based on strategy type"""
        base_conditions = {"timeframe": request.timeframe, "indicators": []}
...
"""
    ...
    async def optimize_strategy(
        self, request: StrategyOptimizationRequest
    ) -> OptimizationResult:
        ...
        try:
            ...
            if "maximize_sharpe" in request.optimization_goals:
                ...
                recommendations.append(
                    "Increased take-profit to 6% improves risk-adjusted returns"
                )
            ...
            if "minimize_drawdown" in request.optimization_goals:
                ...
                recommendations.append(
                    "Tighter stop-loss at 1.5% reduces maximum drawdown"
                )
            ...
            if "maximize_returns" in request.optimization_goals:
                ...
                recommendations.append(
                    "Increased position size and take-profit targets boost returns"
                )
    ...
    async def summarize_backtest(
        self, request: BacktestSummaryRequest, backtest_data: Dict[str, Any]
    ) -> BacktestSummary:
        ...
        try:
            ...
            if sharpe_ratio > 1.5:
                executive_summary += (
                    "The strategy demonstrates strong risk-adjusted performance."
                )
            elif sharpe_ratio > 1.0:
                executive_summary += (
                    "The strategy shows acceptable risk-adjusted returns."
                )
            else:
    ...
    def _generate_backtest_recommendations(self, metrics: Dict[str, Any]) -> List[str]:
        ...
        if total_return > 0:
            recommendations.append(
                "Strategy shows positive returns - consider paper trading"
            )
        else:
        ...
        if sharpe_ratio < 1.0:
            recommendations.append(
                "Consider optimizing parameters to improve risk-adjusted returns"
            )

        recommendations.append(
            "Continue monitoring performance with additional backtests"
        )
        recommendations.append(
            "Consider implementing the strategy with small position sizes initially"
        )
