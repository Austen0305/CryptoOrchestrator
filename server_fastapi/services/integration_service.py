...
class IntegrationService:
        ...
        if circuit_breaker_available:
            self.circuit_breakers = {
                "freqtrade": CircuitBreaker(
                    name="freqtrade_integration", failure_threshold=5, timeout=60
                ),
                "jesse": CircuitBreaker(
                    name="jesse_integration", failure_threshold=5, timeout=60
                ),
            }
    ...
    async def start_integration(self, name: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                "message": f"Integration {name} started successfully",
                "pid": process.pid,
            }
