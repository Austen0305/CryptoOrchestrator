        ...
        try:
            ...
            for _ in range(config.num_simulations):
                ...
                daily_returns = np.random.normal(
                    mean_daily_return, std_daily_return, config.time_horizon_days
                )
            ...
            tail_returns_95 = simulated_returns[
                simulated_returns <= np.percentile(simulated_returns, 5)
            ]
            if len(tail_returns_95) > 0:
                cvar_95 = (
                    abs(np.mean(tail_returns_95))
                    if np.mean(tail_returns_95) < 0
                    else 0.0
                )
            else:
                cvar_95 = var_95
            ...
            expected_shortfall = (
                abs(np.mean(negative_returns)) if len(negative_returns) > 0 else 0.0
            )
        ...
        try:
            ...
            return {
                ...
                "expected_time_to_ruin": (
                    float(expected_time_to_ruin) if expected_time_to_ruin else None
                ),
