...
class VaRService:
        ...
        try:
            ...
            if config.method == "historical":
                return self._calculate_historical_var(
                    returns_array, portfolio_value, config
                )
    ...
    def _calculate_historical_var(
        self, returns: np.ndarray, portfolio_value: float, config: VaRConfig
    ) -> VaRResult:
        ...
        tail_returns = (
            sorted_returns[:threshold_idx] if threshold_idx > 0 else sorted_returns[:1]
        )

        if len(tail_returns) > 0:
            cvar_return = (
                abs(np.mean(tail_returns)) if np.mean(tail_returns) < 0 else 0.0
            )
    ...
    def _calculate_parametric_var(
        ...
        z_scores = {0.90: 1.28, 0.95: 1.65, 0.99: 2.33, 0.975: 1.96, 0.995: 2.58}
        ...
        if z_score is None:
            ...
            if config.confidence_level < 0.95:
                z_score = 1.28 + (config.confidence_level - 0.90) * (1.65 - 1.28) / (
                    0.95 - 0.90
                )
            elif config.confidence_level < 0.99:
                z_score = 1.65 + (config.confidence_level - 0.95) * (2.33 - 1.65) / (
                    0.99 - 0.95
                )
            else:
    ...
    def _calculate_monte_carlo_var(
        ...
        tail_returns = (
            sorted_simulated[:threshold_idx]
            if threshold_idx > 0
            else sorted_simulated[:1]
        )
