...
class DrawdownKillSwitch:
    ...
    async def _check_drawdown(self) -> None:
        ...
        try:
            ...
            max_drawdown = max(
                self.state.max_drawdown_percent if self.state else 0.0,
                current_drawdown
            )
            ...
            if not self.state.active:
                ...
                if self.config.auto_recovery and current_drawdown <= self.config.recovery_threshold_percent:
                    await self._deactivate_kill_switch(current_drawdown, current_value)
        
        except Exception as e:
            logger.error(f"Error checking drawdown: {e}")
    
    async def _activate_kill_switch(self, drawdown_percent: float, current_value: float) -> None:
        ...
        try:
            ...
            if self.config.shutdown_all_bots and self.bot_service:
                try:
                    ...
                    logger.critical("Stopping all active bots due to drawdown kill switch")
    ...
    async def _deactivate_kill_switch(self, drawdown_percent: float, current_value: float) -> None:
    ...
    async def manually_activate(self, reason: str = "Manual activation") -> bool:
        ...
        try:
            current_value = await self._get_current_value()
            current_drawdown = ((self.peak_value - current_value) / self.peak_value) * 100 if self.peak_value > 0 else 0.0
