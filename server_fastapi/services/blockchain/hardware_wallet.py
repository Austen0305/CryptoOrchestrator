...
try:
    ...
    logger.warning(
        "Hardware wallet library not available. Install with: pip install ledgercomm trezor"
    )
...
class HardwareWalletService:
    ...
    def get_device(self, device_id: str) -> HardwareWallet | None:
        """Get device by ID"""
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        ledger_count = sum(
            1 for d in connected_devices if d.wallet_type == HardwareWalletType.LEDGER
        )
        trezor_count = sum(
            1 for d in connected_devices if d.wallet_type == HardwareWalletType.TREZOR
        )
