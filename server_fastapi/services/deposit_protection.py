...
class DepositProtectionService:
    ...
    async def check_deposit_consistency(
        ...
        try:
            ...
            # Calculate totals
            total_deposited = sum(d.net_amount for d in deposits if d.status == TransactionStatus.COMPLETED.value)
            pending_deposits = [d for d in deposits if d.status == TransactionStatus.PENDING.value]
            processing_deposits = [d for d in deposits if d.status == TransactionStatus.PROCESSING.value]
            failed_deposits = [d for d in deposits if d.status == TransactionStatus.FAILED.value]
            
            # Check for orphaned deposits (payment_intent_id but no completion)
            orphaned = [
                d for d in deposits
                if d.payment_intent_id
                and d.status in [TransactionStatus.PENDING.value, TransactionStatus.PROCESSING.value]
                and (datetime.utcnow() - d.created_at) > timedelta(hours=24)
            ]
            
            return {
                "total_deposits": len(deposits),
                "completed_deposits": len([d for d in deposits if d.status == TransactionStatus.COMPLETED.value]),
        ...
        try:
            ...
            try:
                ...
                return {
                    ...
                    "is_reconciled": is_verified and transaction.status == TransactionStatus.COMPLETED.value
                }
            except ImportError:
                return {
                    "found": True,
