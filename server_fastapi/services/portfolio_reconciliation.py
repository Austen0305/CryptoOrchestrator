...
class PortfolioReconciliationService:
    ...
    async def reconcile_portfolio(self, user_id: str) -> Dict:
        ...
        try:
            ...
            logger.error(f"Portfolio reconciliation failed for user {user_id}: {e}", exc_info=True)
            return {
                'user_id': user_id,
                'status': 'error',
                'error': str(e)
    ...
    async def reconcile_all_portfolios(self) -> List[Dict]:
        ...
        try:
            ...
            discrepancy_count = sum(1 for r in results if r['status'] == 'discrepancies_found')
