...
class WalletTradingIntegration:
    ...
    async def reserve_funds_for_trade(
        ...
        try:
            ...
            if wallet.available_balance < amount:
                logger.warning(f"Insufficient balance for trade: user {user_id}, required {amount}, available {wallet.available_balance}")
                return False
    ...
    async def release_funds_from_trade(
        ...
        try:
            ...
            if wallet.locked_balance < amount:
                logger.warning(f"Insufficient locked balance: user {user_id}, required {amount}, locked {wallet.locked_balance}")
                return False
    ...
    async def process_trade_execution(
        ...
        try:
            ...
            if wallet.locked_balance < total_cost:
                logger.error(f"Insufficient locked balance for trade execution: user {user_id}")
                return False
            ...
            await self.db.commit()
            logger.info(f"Processed trade execution in wallet: user {user_id}, trade {trade.id}")
            return True
    ...
    async def process_trade_profit(
        ...
        try:
            ...
            await self.db.commit()
            logger.info(f"Added trade profit to wallet: user {user_id}, profit {profit} {currency}")
            return True
