...
from .accounting_adapters import OAuthCredentials, QuickBooksAdapter, XeroAdapter
...
class AccountingConnectionService:
    ...
    async def complete_oauth_flow(
        ...
        if not connection:
            connection = AccountingConnection(
                ...
                refresh_token=self._encrypt_token(credentials.refresh_token)
            ...
            if credentials.refresh_token:
                connection.refresh_token = self._encrypt_token(
    ...
    async def log_sync(
        ...
        stmt = select(AccountingConnection).where(
            AccountingConnection.id == connection_id
        )
        result = await self.db.execute(stmt)
    ...
    def _get_adapter(self, system: AccountingSystem) -> Any | None:
