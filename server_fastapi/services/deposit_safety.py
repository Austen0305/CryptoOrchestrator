...
class DepositSafetyService:
    ...
    async def _validate_deposit_internal(
        ...
        if amount < self.min_deposit:
            errors.append(
                f"Deposit amount {amount} is below minimum {self.min_deposit}"
            )
        ...
        if payment_intent_id:
            ...
            if existing_transaction:
                if existing_transaction.status == TransactionStatus.COMPLETED.value:
                    errors.append(
                        f"Payment intent {payment_intent_id} has already been processed"
                    )
                    ...
                    errors.append(
                        f"Payment intent {payment_intent_id} is already being processed"
                    )
            ...
            try:
                from .fraud_detection.fraud_detection_service import (
                ...
                if (
                    fraud_analysis.get("is_fraud")
                    or fraud_analysis.get("recommendation") == "block"
                ):
                    errors.append(
    ...
    async def verify_payment_received(
        ...
        try:
            ...
            received_amount = Decimal(str(payment_intent.get("amount", 0))) / Decimal(
                "100"
            )  # Convert cents to dollars
            if abs(received_amount - expected_amount) > Decimal(
                "0.01"
            ...
            if received_currency != currency.upper():
                return (
                    False,
                    f"Currency mismatch. Expected: {currency}, Received: {received_currency}",
                    None,
                )
            ...
            logger.warning(
                "Stripe service not available, skipping payment verification"
            )
    ...
    async def process_deposit_safely(
        ...
        try:
            ...
            if existing_result and existing_result.get("exists"):
                ...
                logger.info(
                    f"Deposit already processed (idempotency): {payment_intent_id}"
                )
            ...
            if not is_verified:
                logger.error(
                    f"Payment verification failed for {payment_intent_id}: {error_msg}"
                )
            ...
            if existing_transaction:
                if existing_transaction.status == TransactionStatus.COMPLETED.value:
                    ...
                    logger.info(
                        f"Deposit already completed: transaction {existing_transaction.id}"
                    )
                    return True, existing_transaction.id, None
                elif existing_transaction.status == TransactionStatus.PROCESSING.value:
                    # Still processing - don't duplicate
                    logger.warning(
                        f"Deposit already processing: transaction {existing_transaction.id}"
                    )
                    return (
                        False,
                        existing_transaction.id,
                        "Deposit is already being processed",
                    )
            ...
            wallet.total_deposited += float(
                amount
            )  # Track total deposited (before fee)
