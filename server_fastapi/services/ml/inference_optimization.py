...
class MLInferenceOptimizationService:
    ...
    def quantize_model(
        ...
        import os
        original_size = os.path.getsize(model_path) / (1024 * 1024) if os.path.exists(model_path) else 0.0
    ...
    def prune_model(
        ...
        optimized_size = original_size * (1 - sparsity * 0.7)  # ~70% of sparsity translates to size reduction
    ...
    def load_onnx_model(
        ...
        try:
            ...
            providers = ["CUDAExecutionProvider", "CPUExecutionProvider"] if (use_gpu or self.use_gpu) else ["CPUExecutionProvider"]
    ...
    def infer_onnx(
        ...
        try:
            ...
            inputs = {
                name: np.array(data) for name, data in input_data.items()
            }
    ...
    def get_optimized_model(self, model_id: str) -> Optional[OptimizedModel]:
