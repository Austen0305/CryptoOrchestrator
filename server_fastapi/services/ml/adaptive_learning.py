...
class AdaptiveLearningService:
    ...
    def get_pattern_success_rate(self, pattern_key: str) -> float:
        ...
        if (
            pattern_key not in self.pattern_history
            or not self.pattern_history[pattern_key]
        ):
    ...
    def adapt_strategy_parameters(
        ...
        try:
            ...
            regime_trades = [
                t
                for t in historical_performance
            ...
            volatility = (
                np.std([t.get("pnl", 0) for t in regime_trades])
            ...
            if market_regime == "bull":
                return {
                    "confidence_threshold": max(
                        0.6, min(0.75, 0.7 - (win_rate - 0.6) * 0.2)
                    ),
                    "position_size_multiplier": max(
                        0.8, min(1.5, 1.0 + (win_rate - 0.6) * 0.5)
                    ),
                    "stop_loss_pct": max(0.015, min(0.03, 0.02 + volatility * 0.5)),
                    "take_profit_pct": max(
                        0.04, min(0.08, 0.05 + (win_rate - 0.6) * 0.1)
                    ),
                }
            elif market_regime == "bear":
                return {
                    "confidence_threshold": max(
                        0.7, min(0.85, 0.75 + (1 - win_rate) * 0.2)
                    ),
                    "position_size_multiplier": max(
                        0.5, min(1.0, 0.8 - (1 - win_rate) * 0.3)
                    ),
                    "stop_loss_pct": max(0.01, min(0.025, 0.015 - volatility * 0.3)),
                    "take_profit_pct": max(
                        0.02, min(0.05, 0.03 - (1 - win_rate) * 0.05)
                    ),
                }
            else:  # ranging
                return {
