...
class NeuralNetworkEngine:
    ...
    def get_recent_accuracy(self) -> float:
        ...
        correct = sum(
            1 for p in self.recent_predictions if p["predicted"] == p["actual"]
        )
    ...
    async def predict(self, market_data: List[MarketData]) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(market_data) < self.config.input_size:
                raise ValueError(
                    f"Insufficient data for prediction. Need at least {self.config.input_size} data points"
                )
    ...
    async def save_model(self, bot_id: str) -> None:
        ...
        try:
            ...
            logger.info(
                f"Neural network model saved successfully for bot {bot_id} at {model_path}"
            )
        except Exception as error:
    ...
    async def load_model(self, bot_id: str) -> bool:
        ...
        try:
            ...
            if os.path.exists(model_path):
                ...
                self.is_trained = True
                logger.info(
                    f"Neural network model loaded successfully for bot {bot_id}"
                )
                return True
            else:
            ...
            logger.warning(
                f"Failed to load neural network model for bot {bot_id}: {error}"
            )
