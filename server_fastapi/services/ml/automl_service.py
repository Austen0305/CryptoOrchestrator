...
try:
    ...
    logger.warning(
        "scikit-optimize not available; Bayesian optimization will be limited."
    )
...
class AutoMLService:
        ...
        try:
            if config.search_strategy == SearchStrategy.GRID:
                ...
                return self._bayesian_optimization(
                    config, objective_function, model_engine
                )
            else:
    ...
    def _grid_search(
        ...
        for i, combination in enumerate(all_combinations):
            ...
            try:
                ...
                trial_results.append({"trial": i + 1, "params": params, "score": score})
            ...
            for name, param_range in config.hyperparameter_ranges.items():
                if param_range.param_type == "int":
                    params[name] = random.randint(
                        int(param_range.min), int(param_range.max)
                    )
                elif param_range.param_type == "float":
    ...
    def _bayesian_optimization(
        ...
        if OPTUNA_AVAILABLE:
            ...
            logger.warning(
                "No Bayesian optimization library available, falling back to random search"
            )
            return self._random_search(config, objective_function, model_engine)
            ...
            for name, param_range in config.hyperparameter_ranges.items():
                if param_range.param_type == "int":
                    ...
                    params[name] = trial.suggest_float(
                        name, param_range.min, param_range.max, step=param_range.step
                    )
                elif param_range.param_type == "categorical":
                    params[name] = trial.suggest_categorical(
                        name, param_range.values or []
                    )

            score = objective_function(params)

            trial_results.append(
                {"trial": trial.number + 1, "params": params, "score": score}
            )

            return score

        study = optuna.create_study(
            direction=config.direction, study_name=f"{config.model_type}_optimization"
        )
