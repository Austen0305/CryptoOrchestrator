...
class QLearningAgent:
    ...
    def get_action(self, state: Dict[str, Any], training: bool = True) -> str:
        ...
        if training and random.random() < self.epsilon:
            action = random.choice([Action.BUY.value, Action.SELL.value, Action.HOLD.value])
        else:
...
class PPOAgent:
    ...
    def __init__(self, config: Optional[RLConfig] = None):
        ...
        if not STABLE_BASELINES_AVAILABLE:
            logger.warning("PPO agent requires stable-baselines3, falling back to Q-learning")
    ...
    def create_trading_env(self, market_data: List[Dict[str, Any]]) -> Any:
        ...
        logger.warning("Custom trading environment not yet implemented, using Q-learning instead")
        return None
    ...
    def train(
        ...
        if not STABLE_BASELINES_AVAILABLE or env is None:
            ...
            return {
                'status': 'error',
                'message': 'PPO training not available'
            }
