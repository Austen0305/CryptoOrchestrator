...
class RealMoneySafetyService:
    ...
    def __init__(self):
        ...
        self.cooldown_after_failed_trade = timedelta(minutes=5)  # Cooldown after failed trade
            ...
            if db is None:
                async with get_db_context() as session:
                    return await self._validate_trade_internal(
                        user_id, exchange, symbol, side, amount, price, session, errors, metadata
                    )
            else:
                return await self._validate_trade_internal(
    ...
    async def _validate_trade_internal(
        ...
        if amount < self.min_trade_amount:
            errors.append(f"Trade amount {amount} is below minimum {self.min_trade_amount}")
        
        if amount > self.max_trade_amount:
            errors.append(f"Trade amount {amount} exceeds maximum {self.max_trade_amount}")
        ...
        if hourly_trades >= self.max_hourly_trades:
            errors.append(f"Hourly trade limit exceeded: {hourly_trades} >= {self.max_hourly_trades}")
        ...
        if recent_failures >= 3:  # 3 failed trades in cooldown period
            errors.append("Too many recent failed trades. Please wait before trading again.")
        ...
        if side == "buy":
            ...
            if wallet:
                required_balance = trade_value * Decimal("1.01")  # Add 1% buffer for fees
            ...
            if db is None:
                async with get_db_context() as session:
                    return await self._validate_withdrawal_internal(
                        user_id, amount, currency, destination, session, errors, metadata
                    )
            else:
                return await self._validate_withdrawal_internal(
    ...
    async def _validate_withdrawal_internal(
        ...
        if wallet.available_balance < amount:
            errors.append(f"Insufficient balance: {wallet.available_balance} < {amount}")
        ...
        try:
            ...
            if operation_type == "trade":
                audit_logger.log_trade(
                    ...
                    success=success,
                    error=error,
                    **{k: v for k, v in details.items() if k not in ["trade_id", "exchange", "symbol", "side", "amount", "price"]}
                )
            else:
