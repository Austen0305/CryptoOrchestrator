...
from .ml.ensemble_engine import EnsembleEngine, ensemble_engine, MarketData, EnsemblePrediction
...
class BacktestingEngine:
    ...
    async def simulate_trading(
        ...
        for i in range(50, len(market_data)):  # Start after enough data for indicators
            ...
            decision: EnsemblePrediction = await self.ensemble_engine.predict(market_data[:i + 1])
            ...
            for pos in positions:
                if pos.side == 'long':
                    ...
                    total_position_value += pos.amount * (2 * pos.entryPrice - current_price)
        ...
        metrics = self.calculate_performance_metrics(trades, equity_curve, config.initialBalance)
    ...
    async def execute_trade(
        ...
        if action == 'buy' and not positions:
            # Enter long position
            position_size = min(
                bot_config.maxPositionSize,
                available_balance * bot_config.riskPerTrade
            )
    ...
    def calculate_performance_metrics(
        ...
        for i in range(1, len(equity_curve)):
            daily_return = (equity_curve[i]["balance"] - equity_curve[i - 1]["balance"]) / equity_curve[i - 1]["balance"]
        ...
        if daily_returns:
            ...
            sharpe_ratio = avg_return / std_dev * (365 ** 0.5) if std_dev > 0 else 0.0  # Annualized
        else:
            sharpe_ratio = 0.0
