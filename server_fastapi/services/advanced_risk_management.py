...
class AdvancedRiskManager:
    ...
    def _parametric_var(
        ...
        expected_shortfall = abs(
            (mean - std * pdf_value / (1 - confidence_level)) * portfolio_value
        )
    ...
    def _monte_carlo_var(
        ...
        return self._historical_var(
            simulated_returns, confidence_level, portfolio_value
        )
    ...
    def check_risk_limits(
        ...
        # Check drawdown
        current_value = account_balance + sum(
        ...
        if self.peak_value > 0:
            ...
            if current_drawdown > self.risk_limits.max_drawdown:
                violations.append(
                    f"Drawdown {current_drawdown:.1%} exceeds limit {self.risk_limits.max_drawdown:.1%}"
                )
            elif current_drawdown > self.risk_limits.max_drawdown * 0.8:
                warnings.append(
                    f"Drawdown {current_drawdown:.1%} approaching limit {self.risk_limits.max_drawdown:.1%}"
                )
    ...
    def analyze_portfolio_risk(
        ...
        for pos in positions:
            ...
            if stop_loss:
                ...
                reward_amount = (
                    abs((take_profit - entry_price) * size) if take_profit else 0
                )
        ...
        if returns_history is not None and len(returns_history) > 0:
            var_95, _ = self.calculate_var(
                returns_history, 0.95, VaRMethod.HISTORICAL, total_value
            )
            var_99, expected_shortfall = self.calculate_var(
                returns_history, 0.99, VaRMethod.HISTORICAL, total_value
            )
