...
class OrderBookSnapshot:
    ...
    def __init__(self, pair: str, bids: List[Tuple[float, float]], asks: List[Tuple[float, float]]):
        self.pair = pair
...
class OrderBookDelta:
    ...
    def __init__(self, pair: str, sequence: int):
        ...
        self.bid_updates: List[Tuple[float, float]] = []  # (price, quantity) - 0 quantity means removal
        self.ask_updates: List[Tuple[float, float]] = []  # (price, quantity) - 0 quantity means removal
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            "sequence": self.sequence,
            "timestamp": self.timestamp,
            "bid_updates": [[float(price), float(qty)] for price, qty in self.bid_updates],
            "ask_updates": [[float(price), float(qty)] for price, qty in self.ask_updates],
        }
...
class HFTOrderBookService:
    ...
    async def get_snapshot(self, pair: str) -> Optional[OrderBookSnapshot]:
    ...
    def get_latency_stats(self, pair: str) -> Optional[Dict]:
