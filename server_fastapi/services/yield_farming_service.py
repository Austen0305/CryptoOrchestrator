...
class YieldFarmingService:
    ...
    async def create_farming_position(
        ...
        if amount < pool["min_deposit"]:
            raise ValueError(f"Amount {amount} below minimum deposit {pool['min_deposit']}")
        
        # Check risk limits
        if risk_limit and pool["risk_score"] > risk_limit:
            raise ValueError(f"Pool risk score {pool['risk_score']} exceeds limit {risk_limit}")
    ...
    async def rebalance_positions(
        ...
        for pool_id, target_pct in target_allocation.items():
            current_position = next((p for p in positions if p["pool_id"] == pool_id), None)
            current_pct = (current_position["current_value"] / total_value * 100) if current_position and total_value > 0 else 0
