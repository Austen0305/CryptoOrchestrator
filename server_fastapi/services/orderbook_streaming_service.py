                ...
                try:
                    ...
                    if order_book:
                            ...
                            for callback in list(self.active_streams[pair]):
                                try:
                                    if asyncio.iscoroutinefunction(callback):
                                        await asyncio.wait_for(callback(order_book), timeout=5.0)
                                    else:
                                        callback(order_book)
                                except asyncio.TimeoutError:
                        ...
                        if consecutive_errors >= max_consecutive_errors:
                            logger.error(f"Too many consecutive errors for {pair}, stopping stream")
                            break
                    
                    # Wait for next update interval with exponential backoff on errors
                    wait_time = self.update_intervals.get(pair, 1.0)
                    if consecutive_errors > 0:
                        wait_time = min(wait_time * backoff_seconds, 30.0)  # Max 30 seconds
                        backoff_seconds = min(backoff_seconds * 2, 16)  # Exponential backoff, max 16x
