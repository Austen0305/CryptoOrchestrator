...
class FraudDetectionService:
        ...
        try:
            if db is None:
                async with get_db_context() as session:
                    return await self._analyze_internal(user_id, transaction_type, amount, currency, metadata, session)
            else:
                return await self._analyze_internal(user_id, transaction_type, amount, currency, metadata, db)
    ...
    async def _analyze_internal(
        ...
        amount_risk = await self._check_amount_anomaly(user_id, amount, transaction_type, db)
        ...
        behavioral_risk = await self._check_behavioral_pattern(user_id, transaction_type, amount, db)
        ...
        if metadata and metadata.get("ip_address"):
            geo_risk = await self._check_geographic_anomaly(user_id, metadata.get("ip_address"), db)
        ...
        recommendation = "block" if is_fraud else ("review" if risk_score >= 0.5 else "allow")
    ...
    async def _check_velocity(
        ...
        try:
            window_start = datetime.utcnow() - timedelta(hours=self.velocity_window_hours)
            ...
            is_suspicious = (
                transaction_count > self.max_transactions_per_hour * self.velocity_window_hours or
                hourly_count > self.max_transactions_per_hour
            )
            
            return {
                "is_suspicious": is_suspicious,
                "message": f"High transaction velocity detected: {hourly_count} transactions/hour, {transaction_count} in last {self.velocity_window_hours} hours" if is_suspicious else "Normal transaction velocity",
                "details": {
            ...
            return {"is_suspicious": False, "message": "Velocity check unavailable", "details": {}}
    ...
    async def _check_amount_anomaly(
        ...
        try:
            ...
            if not stats or not stats.avg_amount:
                ...
                return {
                    "is_suspicious": is_suspicious,
                    "message": f"Large transaction amount: {amount} (no history for comparison)" if is_suspicious else "Amount check passed",
                    "details": {"amount": float(amount), "threshold": float(self.max_amount_per_day)}
                }
            ...
            threshold = avg_amount + (stddev_amount * 3) if stddev_amount > 0 else avg_amount * 5
            
            is_suspicious = amount > threshold or amount > self.max_amount_per_day
            
            return {
                "is_suspicious": is_suspicious,
                "message": f"Unusual transaction amount: {amount} (avg: {avg_amount:.2f}, threshold: {threshold:.2f})" if is_suspicious else "Amount within normal range",
                "details": {
                    "amount": float(amount),
            ...
            return {"is_suspicious": False, "message": "Amount check unavailable", "details": {}}
    ...
    async def _check_behavioral_pattern(
        ...
        try:
            ...
            return {
                "is_suspicious": is_suspicious,
                "message": f"Behavioral deviation detected: {deviation*100:.1f}% from average" if is_suspicious else "Transaction matches behavioral pattern",
                "details": {
            ...
            return {"is_suspicious": False, "message": "Behavioral check unavailable", "details": {}}
    ...
    def _check_time_anomaly(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "is_suspicious": is_suspicious,
                "message": f"Unusual transaction time: {current_hour}:00 UTC" if is_suspicious else "Normal transaction time",
                "details": {"hour": current_hour}
            }
        except Exception as e:
            logger.error(f"Error checking time anomaly: {e}")
            return {"is_suspicious": False, "message": "Time check unavailable", "details": {}}
    ...
    async def _get_risk_profile_internal(
        ...
        return {
            ...
            "total_volume": float(stats.total_volume) if stats and stats.total_volume else 0.0,
            "analyzed_at": datetime.utcnow().isoformat()
        }
