...
class SmartAlertsService:
        ...
        while True:
            try:
                ...
                if rule_id in self.last_triggered:
                    time_since_last = (datetime.utcnow() - self.last_triggered[rule_id]).total_seconds()
    ...
    async def _execute_actions(self, alert: SmartAlert, actions: List[str]) -> None:
        ...
        for action in actions:
            try:
                ...
                logger.error(f"Error executing action {action} for alert {alert.id}: {e}")
    ...
    def get_active_alerts(self, priority: Optional[AlertPriority] = None) -> List[SmartAlert]:
        """Get active alerts"""
    ...
    def get_alert_history(self, rule_id: Optional[str] = None, limit: int = 100) -> List[SmartAlert]:
        """Get alert history"""
