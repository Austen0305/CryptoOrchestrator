...
class StrategySwitchingService:
    ...
    async def _monitor_and_switch(
        self, bot_id: str, config: StrategySwitchConfig
    ) -> None:
            ...
            try:
                ...
                if current_strategy:
                    performance = await self._evaluate_strategy_performance(
                        bot_id, current_strategy, config
                    )

                    if performance:
                        self.strategy_performances.setdefault(bot_id, []).append(
                            performance
                        )
                        ...
                        if config.switch_on_performance:
                            should_switch = await self._should_switch_on_performance(
                                bot_id, performance, config
                            )

                            if should_switch:
                                await self._switch_strategy(
                                    bot_id, current_strategy, regime, "performance"
                                )

                        if config.switch_on_regime_change:
                            should_switch = await self._should_switch_on_regime(
                                bot_id, current_strategy, regime, config
                            )

                            if should_switch:
                                await self._switch_strategy(
                                    bot_id, current_strategy, regime, "regime_change"
                                )
    ...
    async def _switch_strategy(
        self, bot_id: str, from_strategy: str, regime: MarketRegime, reason: str
    ) -> bool:
    ...
    async def _select_strategy_for_regime(
        self, regime: MarketRegime, current_strategy: str
    ) -> str:
