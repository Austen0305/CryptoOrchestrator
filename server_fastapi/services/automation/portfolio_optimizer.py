...
class PortfolioOptimizationAdvisor:
    ...
    async def _calculate_portfolio_metrics(
        ...
        diversification = 1 - sum(w ** 2 for w in allocations.values()) if allocations else 0
    ...
    async def _generate_recommendations_for_goal(
        ...
        if goal == OptimizationGoal.MAXIMIZE_RETURNS:
            recs = await self._recommend_maximize_returns(portfolio, metrics, preferences)
            recommendations.extend(recs)
            ...
            recs = await self._recommend_maximize_sharpe(portfolio, metrics, preferences)
            recommendations.extend(recs)
        
        elif goal == OptimizationGoal.MINIMIZE_DRAWDOWN:
            recs = await self._recommend_minimize_drawdown(portfolio, metrics, preferences)
            recommendations.extend(recs)
        
        elif goal == OptimizationGoal.DIVERSIFICATION:
            recs = await self._recommend_diversification(portfolio, metrics, preferences)
            recommendations.extend(recs)
        
        elif goal == OptimizationGoal.TAX_OPTIMIZATION:
            recs = await self._recommend_tax_optimization(portfolio, metrics, preferences)
            recommendations.extend(recs)
        ...
        if concentration > 0.5:
            recommendations.append(
                OptimizationRecommendation(
                    ...
                    actions=[
                        {"type": "rebalance", "description": "Rebalance to reduce concentration"}
                    ],
        ...
        if diversification < 0.7:
            recommendations.append(
                OptimizationRecommendation(
                    ...
                    actions=[
                        {"type": "add_assets", "description": "Add uncorrelated assets to portfolio"}
                    ],
        ...
        if len(allocations) > 1:
            recommendations.append(
                OptimizationRecommendation(
                    ...
                    actions=[
                        {"type": "optimize_allocation", "description": "Rebalance using risk parity"},
                        {"type": "adjust_risk", "description": "Fine-tune risk parameters"}
                    ],
    ...
    async def _recommend_minimize_drawdown(
        ...
        recommendations.append(
            OptimizationRecommendation(
                ...
                actions=[
                    {"type": "add_stop_loss", "description": "Set stop-loss at 5% for all positions"},
                    {"type": "reduce_position_size", "description": "Reduce maximum position size"}
                ],
                confidence_score=0.80,
        ...
        if num_assets < 5:
            recommendations.append(
                OptimizationRecommendation(
                    ...
                    current_state={"num_assets": num_assets, "diversification": diversification},
                    recommended_state={"num_assets": 7, "diversification": 0.85},
                    expected_impact={"diversification": 0.15, "volatility": -0.12},
                    actions=[
                        {"type": "add_assets", "description": "Add 2-3 uncorrelated assets"}
                    ],
                    confidence_score=0.75,
    ...
    async def _recommend_tax_optimization(
        ...
        recommendations.append(
            OptimizationRecommendation(
                ...
                actions=[
                    {"type": "harvest_losses", "description": "Sell losing positions and rebuy similar assets"}
                ],
