...
class AutoHedgingService:
    ...
    async def _execute_hedge(
        ...
        try:
            if config.strategy == HedgingStrategy.DELTA_NEUTRAL:
                hedge_pos = await self._delta_neutral_hedge(portfolio_id, exposure, config)
            elif config.strategy == HedgingStrategy.PAIRS_TRADING:
                hedge_pos = await self._pairs_trading_hedge(portfolio_id, exposure, config)
            elif config.strategy == HedgingStrategy.CORRELATION_HEDGE:
                hedge_pos = await self._correlation_hedge(portfolio_id, exposure, config)
            ...
            if hedge_pos:
                ...
                logger.info(f"Executed hedge for portfolio {portfolio_id}: {hedge_pos.reason}")
    ...
    async def _correlation_hedge(
        ...
        return HedgePosition(
            symbol="BTC",
            side="sell" if exposure > 0 else "buy",
            amount=abs(exposure) * config.hedge_ratio * 0.8,  # Partial hedge based on correlation
    ...
    def get_active_hedges(self, portfolio_id: Optional[str] = None) -> List[HedgePosition]:
