...
class MarketplaceThresholdService:
    ...
    async def _get_metric_value(self, threshold: AnalyticsThreshold) -> float | None:
        ...
        try:
            ...
            if threshold_type == ThresholdType.PROVIDER.value:
                ...
                metric_map = {
                    ...
                    ThresholdMetric.AVERAGE_RATING.value: analytics.get(
                        "average_rating"
                    ),
                    ThresholdMetric.FOLLOWER_COUNT_CHANGE.value: analytics.get(
                        "follower_count"
                    ),
                }
                ...
                metric_map = {
                    ThresholdMetric.INDICATOR_REVENUE_DROP_PERCENT.value: analytics.get(
                        "total_revenue"
                    ),
                    ThresholdMetric.INDICATOR_PURCHASE_COUNT_CHANGE.value: analytics.get(
                        "total_purchases"
                    ),
                    ThresholdMetric.INDICATOR_AVERAGE_RATING.value: analytics.get(
                        "average_rating"
                    ),
                }
                ...
                metric_map = {
                    ThresholdMetric.REVENUE_DROP_PERCENT.value: copy_trading.get(
                        "platform_revenue", 0.0
                    ),
                    ThresholdMetric.TOTAL_PROVIDERS_CHANGE.value: copy_trading.get(
                        "total_providers", 0
                    ),
                }
                ...
                metric_map = {
                    ThresholdMetric.INDICATOR_REVENUE_DROP_PERCENT.value: indicators.get(
                        "platform_revenue", 0.0
                    ),
                    ThresholdMetric.TOTAL_INDICATORS_CHANGE.value: indicators.get(
                        "total_indicators", 0
                    ),
                }
