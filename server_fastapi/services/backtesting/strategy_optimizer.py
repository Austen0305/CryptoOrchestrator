...
class StrategyOptimizer:
    ...
    async def optimize_strategy(
        ...
        for generation in range(config.generations):
            ...
            fitness_scores = await self._evaluate_population(population, historical_data, bot_id)
            ...
            backtest_result = await self.backtesting_engine.run_backtest(backtest_config, historical_data)
            ...
            population = self._create_next_generation(population, fitness_scores, config)
    ...
    async def run_parameter_sweep(
        ...
        param_combinations = self._generate_parameter_combinations(param_ranges, max_combinations)
        ...
        for i, params in enumerate(param_combinations):
            ...
            try:
                ...
                fitness_score = backtest_result.sharpeRatio + (backtest_result.totalReturn * 0.1)
        ...
        return {
            ...
            "fitness_std": math.sqrt(sum((x - (sum(fitness_scores) / len(fitness_scores))) ** 2 for x in fitness_scores) / len(fitness_scores)) if fitness_scores else 0,
    ...
    def _initialize_population(self, config: OptimizationConfig) -> List[Dict[str, Any]]:
        ...
        for _ in range(config.population_size):
            ...
            for param_name, param_range in config.parameters.items():
                if param_range.step:
                    # Discrete parameter
                    steps = int((param_range.max - param_range.min) / param_range.step) + 1
                    individual[param_name] = param_range.min + random.randint(0, steps - 1) * param_range.step
                else:
        ...
        for params in population:
            try:
                ...
                fitness_score = (
                    ...
                    (backtest_result.profitFactor if backtest_result.profitFactor != float('inf') else 10) * 0.1
                )
    ...
    def _create_next_generation(
        ...
        while len(new_population) < config.population_size:
            ...
            parent1 = self._tournament_selection(population, fitness_scores, config.tournament_size)
            parent2 = self._tournament_selection(population, fitness_scores, config.tournament_size)
    ...
    def _generate_parameter_combinations(
        ...
        for param_name, param_range in param_ranges.items():
            if param_range.step:
                ...
                param_values[param_name] = [param_range.min + i * step for i in range(10)]
