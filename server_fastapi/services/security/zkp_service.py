...
try:
    ...
    ZKP_AVAILABLE = False
    logger.warning("ZKP library not available. Install with: pip install zokrates-python or similar")
...
class ZKPService:
    ...
    def get_balance_proof(self, wallet_address: str) -> Optional[BalanceProof]:
        """Get balance proof for a wallet"""
        return self.balance_proofs.get(wallet_address)
    
    def list_proofs(self, wallet_address: Optional[str] = None) -> list[ZKPProof]:
    ...
    def export_proof(self, proof_id: str) -> Optional[Dict[str, Any]]:
