...
class WithdrawalWhitelistService:
        ...
        try:
            ...
            if db is None:
                async with get_db_context() as session:
                    return await self._add_address_internal(
                        user_id, address, currency, label, session
                    )
            else:
                return await self._add_address_internal(
                    user_id, address, currency, label, db
                )
    ...
    async def _add_address_internal(
        ...
        existing = next(
            (
                addr
                for addr in data["withdrawal_whitelist"]
    ...
    async def remove_withdrawal_address(
        ...
        try:
            if db is None:
                async with get_db_context() as session:
                    return await self._remove_address_internal(
                        user_id, address, currency, session
                    )
            else:
                return await self._remove_address_internal(
                    user_id, address, currency, db
                )
        except Exception as e:
    ...
    async def get_whitelist(
        ...
        try:
            if db is None:
                async with get_db_context() as session:
                    return await self._get_whitelist_internal(
                        user_id, currency, session
                    )
            else:
