...
class TradingSafetyService:
    ...
    def validate_trade(
        ...
        current_exposure = self._calculate_portfolio_exposure(current_positions, account_balance)
    ...
    def check_slippage(
        ...
        return {
            ...
            'reason': 'Acceptable' if acceptable else f'Slippage {slippage_pct:.2%} too high'
        }
    ...
    def _check_daily_reset(self):
        ...
        if now.date() > self.last_reset.date():
            ...
            if self.kill_switch_active:
                ...
                self.kill_switch_active = False
                self.kill_switch_reason = None
                logger.warning(f"Kill switch auto-reset (new day): Previous reason: {old_reason}")
