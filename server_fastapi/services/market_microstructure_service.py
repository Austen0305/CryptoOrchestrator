...
class MarketMicrostructure:
    ...
    trade_flow_imbalance: float  # (buy_volume - sell_volume) / (buy_volume + sell_volume)
...
class MarketMicrostructureService:
    ...
    async def calculate_microstructure(
        ...
        bid_ask_volatility = abs(bid_ask_spread - (best_ask - best_bid)) / mid_price if mid_price > 0 else 0.0
    ...
    def _estimate_price_impact(
        ...
        avg_impact = (buy_impact + sell_impact) / (2 * target_volume) if target_volume > 0 else 0.0
    ...
    def _calculate_effective_spread(
        ...
        if total_depth > 0:
            effective_bid = sum(price * qty for price, qty in bids[:5]) / bid_depth if bid_depth > 0 else best_bid
            effective_ask = sum(price * qty for price, qty in asks[:5]) / ask_depth if ask_depth > 0 else best_ask
    ...
    async def get_microstructure(self, pair: str) -> Optional[MarketMicrostructure]:
