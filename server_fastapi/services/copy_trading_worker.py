...
class CopyTradingWorker:
                ...
                try:
                    ...
                    trades_stmt = select(Trade).where(
                        and_(
                            ...
                            Trade.mode == "paper",  # Only copy paper trades by default
                    ...
                    for trade in new_trades:
                        ...
                        try:
                            ...
                            logger.error(f"Error copying trade {trade.id}: {e}", exc_info=True)
                    
                    # Update last check time
                    if new_trades:
                        self.last_check_time[follow.trader_id] = new_trades[-1].timestamp
                    else:
                        self.last_check_time[follow.trader_id] = datetime.utcnow()
                        
                except Exception as e:
                    logger.error(f"Error processing trades for trader {follow.trader_id}: {e}", exc_info=True)
                    continue
    ...
    async def _copy_trade_automatically(self, follow: Follow, trade: Trade):
        ...
        try:
            ...
            logger.info(f"Automatically copied trade {trade.id} for follower {follow.follower_id}")
            return copied_trade
            
        except Exception as e:
            logger.error(f"Error automatically copying trade {trade.id}: {e}", exc_info=True)
            raise
