...
class DemoModeService:
    ...
    DEMO_FEATURES = {
        ...
        'live_trading': FeatureFlag(name='live_trading', enabled=False, limit=None, message='Live trading requires a license'),
        'basic_strategies': FeatureFlag(name='basic_strategies', enabled=True, limit=3, message='Demo mode limited to 3 strategies'),
        'advanced_strategies': FeatureFlag(name='advanced_strategies', enabled=False, limit=None, message='Advanced strategies require a license'),
        'ml_models': FeatureFlag(name='ml_models', enabled=False, limit=None, message='ML models require a license'),
        'api_access': FeatureFlag(name='api_access', enabled=False, limit=None, message='API access requires a license'),
        'multiple_bots': FeatureFlag(name='multiple_bots', enabled=True, limit=3, message='Demo mode limited to 3 bots'),
        'backtesting': FeatureFlag(name='backtesting', enabled=True, limit=10, message='Demo mode limited to 10 backtests per day'),
    }
    ...
    def set_license(self, license_status: LicenseStatus) -> None:
        ...
        self.is_demo_mode = not license_status.valid or license_status.license_type == LicenseType.TRIAL
    
    def check_feature(self, feature_name: str) -> Dict[str, Any]:
        """Check if a feature is available"""
        if not self.is_demo_mode and self.current_license and self.current_license.valid:
    ...
    def get_demo_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'license_type': self.current_license.license_type if self.current_license else 'demo',
            'license_valid': self.current_license.valid if self.current_license else False,
            'features': self.get_available_features(),
            'limits': self.get_feature_limits(),
            'expires_at': self.current_license.expires_at.isoformat() if self.current_license and self.current_license.expires_at else None
        }
