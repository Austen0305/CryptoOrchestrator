    ...
    async def get_total_revenue(
        ...
        if db is None:
            ...
            async with get_db_context() as session:
                return await self._get_total_revenue_internal(start_date, end_date, session)
        else:
    ...
    async def _get_total_revenue_internal(
        ...
        try:
            ...
            fees_result = await db.execute(
                select(func.sum(WalletTransaction.fee))
            ...
            avg_fee = total_fees / transaction_count if transaction_count > 0 else Decimal("0")
