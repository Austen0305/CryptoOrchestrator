...
from ...services.hft_orderbook_service import (
    hft_orderbook_service,
)
from ...services.market_microstructure_service import (
    market_microstructure_service,
)
...
try:
    ...
    logger.warning(
        "gRPC proto files not generated. Run: python -m grpc_tools.protoc -I. --python_out=. --grpc_python_out=. proto/hft.proto"
    )
    hft_pb2 = None
    hft_pb2_grpc = None
...
class HFTService(hft_pb2_grpc.HFTServiceServicer if hft_pb2_grpc else object):
    ...
    async def GetOrderBookSnapshot(
        ...
        try:
            ...
            if not snapshot:
                ...
                context.set_details(
                    f"Order book snapshot not available for {request.pair}"
                )
    ...
    async def GetMicrostructure(
        ...
        try:
            microstructure = await market_microstructure_service.get_microstructure(
                request.pair
            )
            ...
            if not microstructure:
                context.set_code(grpc.StatusCode.NOT_FOUND)
                context.set_details(
                    f"Microstructure data not available for {request.pair}"
                )
    ...
    async def GetLatencyStats(
        ...
        try:
            ...
            if not stats:
                context.set_code(grpc.StatusCode.NOT_FOUND)
                context.set_details(
                    f"Latency statistics not available for {request.pair}"
                )
