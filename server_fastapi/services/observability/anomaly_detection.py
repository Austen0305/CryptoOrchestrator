...
class AnomalyDetectionService:
    ...
    def _update_baseline(self, key: str, metric_name: str):
        ...
        cutoff_time = datetime.utcnow() - timedelta(minutes=self.baseline_window_minutes)
    ...
    def _detect_anomaly(self, key: str, metric_name: str, value: float, tags: Optional[Dict[str, str]]) -> Optional[Anomaly]:
        ...
        if self.use_isolation_forest and key in self.isolation_forests:
            try:
                ...
                if len(history) >= self.lstm_window_size:
                    recent_values = [v for _, v in list(history)[-self.lstm_window_size:]]
                    ...
                    # Predict
                    prediction = self.isolation_forests[key].predict(scaled[-1:].reshape(1, -1))
        ...
        if self.use_lstm and key in self.lstm_models:
            try:
                ...
                if len(history) >= self.lstm_window_size:
                    ...
                    # Prepare input
                    input_tensor = torch.FloatTensor(scaled_sequence[-self.lstm_window_size:]).unsqueeze(0)
    ...
    def get_baseline(self, metric_name: str, tags: Optional[Dict[str, str]] = None) -> Optional[Baseline]:
    ...
    def train_isolation_forest(self, metric_name: str, tags: Optional[Dict[str, str]] = None):
        """
        ...
        if not self.use_isolation_forest:
            logger.warning("Isolation Forest not available (scikit-learn not installed)")
            return False
        ...
        history = self.metric_history[key]
        if len(history) < self.min_samples:
            logger.warning(f"Insufficient samples for {metric_name}: {len(history)} < {self.min_samples}")
            return False
        ...
        try:
            ...
            logger.error(f"Failed to train Isolation Forest for {metric_name}: {e}", exc_info=True)
            return False
    
    def train_lstm_autoencoder(self, metric_name: str, tags: Optional[Dict[str, str]] = None, epochs: int = 10):
        """
        ...
        if len(history) < self.lstm_window_size * 2:
            logger.warning(f"Insufficient samples for LSTM: {len(history)} < {self.lstm_window_size * 2}")
            return False
    ...
    def auto_train_models(self, metric_name: Optional[str] = None):
        """
        ...
        """
        metrics_to_train = [metric_name] if metric_name else list(self.metric_history.keys())
        ...
        for key in metrics_to_train:
            ...
            if self.use_lstm and len(self.metric_history.get(key, deque())) >= self.lstm_window_size * 2:
