...
class Forecast:
    ...
    predicted_threshold_breach: Optional[datetime] = None
...
class PredictiveAlertingService:
    ...
    def _generate_forecast(self, key: str, metric_name: str, tags: Optional[Dict[str, str]]) -> Optional[Forecast]:
    ...
    def predict_threshold_breach(
        ...
        if forecast.trend == "increasing" and condition in (">", ">="):
            if current_value < threshold:
                ...
                if rate > 0:
                    ...
                    predicted_breach_time = datetime.utcnow() + timedelta(minutes=time_to_breach)
                else:
                    return None
            else:
                return None
        ...
        confidence = 0.7  # Base confidence, could be improved with more sophisticated models
        
        # Determine severity based on time to breach
        minutes_to_breach = (predicted_breach_time - datetime.utcnow()).total_seconds() / 60
    ...
    def get_forecast(self, metric_name: str, tags: Optional[Dict[str, str]] = None) -> Optional[Forecast]:
