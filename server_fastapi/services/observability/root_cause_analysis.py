...
class RootCauseAnalysisService:
    ...
    def __init__(self):
        ...
        self.metric_history: dict[str, list[tuple]] = defaultdict(
            list
        )  # (timestamp, value)
            ...
            for metric_name, metric_value in incident.metrics.items():
                ...
                for corr in correlations:
                    ...
                    if other_metric in self.metric_history:
                        recent_values = [
                            v
                            for t, v in self.metric_history[other_metric]
    ...
    def add_correlation(
        ...
        logger.debug(
            f"Added correlation: {metric_a} <-> {metric_b} ({correlation_score:.2f})"
        )
