...
class SLAService:
    ...
    def record_availability(self, service_name: str, is_up: bool, tags: Optional[Dict[str, str]] = None):
    ...
    def record_latency(self, service_name: str, latency_ms: float, tags: Optional[Dict[str, str]] = None):
    ...
    def record_error(self, service_name: str, is_error: bool, tags: Optional[Dict[str, str]] = None):
    ...
    def record_throughput(self, service_name: str, requests: int, tags: Optional[Dict[str, str]] = None):
    ...
    def calculate_sla(self, sla_name: str) -> Optional[SLAMetric]:
        ...
        if sla_target.sla_type == SLAType.AVAILABILITY:
            ...
            if key in self.availability_data:
                recent_data = [
                    (t, up) for t, up in self.availability_data[key]
                    if window_start <= t <= window_end
                ]
                if recent_data:
                    uptime_count = sum(1 for _, up in recent_data if up)
                    total_count = len(recent_data)
                    current_value = (uptime_count / total_count * 100) if total_count > 0 else 100.0
                else:
            ...
            compliance_percentage = min(100.0, (current_value / sla_target.target_value) * 100) if sla_target.target_value > 0 else 100.0
        
        elif sla_target.sla_type == SLAType.LATENCY:
            # Calculate p95 latency
            if key in self.latency_data:
                recent_latencies = self.latency_data[key][-1000:]  # Last 1000 measurements
                if recent_latencies:
                    sorted_latencies = sorted(recent_latencies)
                    p95_index = int(len(sorted_latencies) * 0.95)
                    current_value = sorted_latencies[p95_index] if p95_index < len(sorted_latencies) else sorted_latencies[-1]
                else:
                    current_value = 0.0
            else:
                current_value = 0.0
            ...
            is_compliant = current_value <= sla_target.target_value
            compliance_percentage = min(100.0, (sla_target.target_value / current_value) * 100) if current_value > 0 else 100.0
        
        elif sla_target.sla_type == SLAType.ERROR_RATE:
            # Calculate error rate percentage
            if key in self.error_data:
                recent_data = [
                    (t, err) for t, err in self.error_data[key]
                    if window_start <= t <= window_end
                ]
                if recent_data:
                    error_count = sum(1 for _, err in recent_data if err)
                    total_count = len(recent_data)
                    current_value = (error_count / total_count * 100) if total_count > 0 else 0.0
                else:
                    current_value = 0.0
            else:
                current_value = 0.0
            ...
            if key in self.throughput_data:
                recent_data = [
                    (t, req) for t, req in self.throughput_data[key]
                    if window_start <= t <= window_end
                ]
                if recent_data:
                    total_requests = sum(req for _, req in recent_data)
                    time_span_minutes = (window_end - window_start).total_seconds() / 60
                    current_value = (total_requests / time_span_minutes * 60) if time_span_minutes > 0 else 0.0  # req/sec
                else:
                    current_value = 0.0
            else:
                current_value = 0.0
            ...
            is_compliant = current_value >= sla_target.target_value
            compliance_percentage = min(100.0, (current_value / sla_target.target_value) * 100) if sla_target.target_value > 0 else 0.0
