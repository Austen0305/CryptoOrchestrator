...
class InstitutionalWalletService:
    ...
    async def create_institutional_wallet(
        ...
        total_signers: int = 1,
        signer_user_ids: Optional[List[int]] = None,
        ...
        try:
            ...
            if wallet_type == WalletType.MULTISIG.value:
                if not multisig_type:
                    raise ValueError("multisig_type required for multi-signature wallets")
            ...
            if signer_user_ids:
                ...
                await self.add_signer(wallet.id, user_id, SignerRole.OWNER.value, user_id)
            ...
            logger.info(
                ...
                extra={"wallet_id": wallet.id, "user_id": user_id, "wallet_type": wallet_type}
            )
    ...
    async def add_signer(
        ...
        try:
            ...
            if existing_result.scalar_one_or_none():
                logger.warning(f"User {signer_user_id} already a signer of wallet {wallet_id}")
                return False
    ...
    async def create_pending_transaction(
        ...
        try:
            ...
            if not await self.has_permission(wallet_id, created_by_user_id, "sign"):
                raise PermissionError("User does not have permission to create transactions")
    ...
    async def sign_transaction(
        ...
        try:
            ...
            if not await self.has_permission(pending_tx.wallet_id, user_id, "sign"):
                raise PermissionError("User does not have permission to sign transactions")
            
            # Check if already signed by this user
            if str(user_id) in pending_tx.signatures:
                logger.warning(f"User {user_id} already signed transaction {transaction_id}")
                return False
    ...
    async def has_permission(
        ...
        try:
            ...
            # Permission mapping
            if permission == "view":
                return role in [SignerRole.OWNER.value, SignerRole.SIGNER.value, SignerRole.VIEWER.value, SignerRole.ADMIN.value]
            elif permission == "sign":
                return role in [SignerRole.OWNER.value, SignerRole.SIGNER.value, SignerRole.ADMIN.value]
    ...
    async def list_wallets(
        ...
        signer_result = await self.db.execute(
            text("SELECT wallet_id FROM wallet_signer_associations WHERE user_id = :user_id"),
            {"user_id": user_id},
        )
        ...
        query = select(InstitutionalWallet).where(
            or_(
                ...
                InstitutionalWallet.id.in_(signer_wallet_ids) if signer_wallet_ids else False,
            )
        )
    ...
    async def export_audit_logs(
        ...
        Returns:
            List of access logs
        """
        query = select(WalletAccessLog).where(
            WalletAccessLog.wallet_id == wallet_id
        )
