...
class ReadReplicaRoutingService:
    ...
    async def initialize(self, primary_url: str, replica_urls: list[str]):
        ...
        try:
            ...
            logger.error(f"Failed to initialize read replica routing: {e}", exc_info=True)
            self._is_initialized = False
    
    async def get_read_session(self) -> Optional[AsyncSession]:
        """
    ...
    async def get_write_session(self) -> Optional[AsyncSession]:
        """
