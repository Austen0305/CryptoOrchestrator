...
class MLInferenceOptimizationService:
    ...
    def quantize_model_int8(
        ...
        try:
            ...
            logger.info(
                f"Quantized model {model_id} to int8: {speedup_factor}x speedup"
            )
    ...
    def prune_model(
        ...
        try:
            ...
            optimization = ModelOptimization(
                model_id=model_id,
                optimization_level=OptimizationLevel.PRUNING_LIGHT
                if pruning_ratio < 0.3
                else OptimizationLevel.PRUNING_AGGRESSIVE,
                original_size_mb=original_size,
                optimized_size_mb=optimized_size,
                speedup_factor=speedup_factor,
                accuracy_loss=accuracy_loss,
            ...
            self.optimized_models[model_id] = optimization
            logger.info(
                f"Pruned model {model_id} by {pruning_ratio * 100}%: {speedup_factor}x speedup"
            )
    ...
    def convert_to_onnx(
        ...
        try:
            ...
            self.optimized_models[model_id] = optimization
            logger.info(
                f"Converted model {model_id} to ONNX: {speedup_factor}x speedup"
            )

            return optimization
        except Exception as e:
            logger.error(
                f"Failed to convert model {model_id} to ONNX: {e}", exc_info=True
            )
            raise
    ...
    def get_optimization_stats(self) -> dict[str, Any]:
        ...
        avg_speedup = (
            sum(o.speedup_factor for o in self.optimized_models.values()) / total_models
            if total_models > 0
            else 0.0
        )
        avg_size_reduction = (
            sum(
                (o.original_size_mb - o.optimized_size_mb) / o.original_size_mb
                for o in self.optimized_models.values()
            )
            / total_models
            if total_models > 0
            else 0.0
