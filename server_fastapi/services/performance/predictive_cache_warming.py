...
class PredictiveCacheWarmingService:
    ...
    def record_access(
        ...
        recent_accesses = [
            a
            for a in self.access_history[user_id]
    ...
    def predict_next_access(
        ...
        if endpoint:
            patterns = (
                {endpoint: patterns.get(endpoint)} if endpoint in patterns else {}
            )
        ...
        for ep, pattern in patterns.items():
            ...
            if should_warm and pattern.frequency > 0.1:  # At least 0.1 requests/hour
                ...
                hours_until_next = (
                    1.0 / pattern.frequency if pattern.frequency > 0 else 1.0
                )
