...
async def execute_bot(self, bot_id: str, user_id: int, session: AsyncSession = None):
    ...
    try:
        ...
        if not is_active:
            logger.warning(f"User {user_id} subscription inactive. Stopping bot {bot_id}")
...
@celery_app.task(
    name="bot_worker.check_subscriptions",
    ignore_result=True
)
