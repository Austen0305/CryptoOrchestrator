...
class SQLAlchemyRepository(BaseRepository[T, int]):
    ...
    async def create(self, session: AsyncSession, data: dict[str, Any]) -> T:
        ...
        if "hashed_password" in create_data and hasattr(
            self.model_class, "password_hash"
        ):
        ...
        stmt = (
            insert(self.model_class).values(**create_data).returning(self.model_class)
        )
        ...
        try:
            ...
            # If a UNIQUE constraint on the primary key failed, return the existing record
            ...
            if "id" in create_data:
                ...
                if obj:
                    ...
                    return obj
            # Re-raise if we couldn't resolve the conflict
            raise
    ...
    async def update(
        ...
        if "hashed_password" in update_data and hasattr(
            self.model_class, "password_hash"
        ):
        ...
        stmt = (
            ...
            .where(self.model_class.id == id, ~self.model_class.is_deleted)
            .values(**update_data)
    ...
    async def delete(self, session: AsyncSession, id: int) -> bool:
        ...
        stmt = (
            update(self.model_class)
            .where(self.model_class.id == id, ~self.model_class.is_deleted)
            .values(is_deleted=True, deleted_at=None)
    ...
    async def exists(self, session: AsyncSession, id: int) -> bool:
        ...
        query = select(self.model_class.id).where(
            self.model_class.id == id, ~self.model_class.is_deleted
        )
        result = await session.execute(query)
