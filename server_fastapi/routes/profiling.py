...
async def get_profiling_stats_endpoint(
    ...
    try:
        ...
        return {
            ...
            "stats": stats,
            "slow_middleware": [
                {"name": name, "avg_time": avg_time}
                for name, avg_time in slow
            ],
            "summary": {
...
async def enable_profiling_endpoint(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to enable profiling: {str(e)}")
...
async def disable_profiling_endpoint(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to disable profiling: {str(e)}")
...
async def clear_profiling_endpoint(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to clear profiling data: {str(e)}")
...
async def log_profiling_summary_endpoint(
    ...
    try:
        ...
        return {"status": "logged", "message": "Profiling summary logged to server logs"}
    except Exception as e:
