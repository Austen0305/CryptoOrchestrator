...
async def get_candle_history(
    ...
    try:
        ...
        candles = [CandleDTO(ts=r.ts, open=r.open, high=r.high, low=r.low, close=r.close, volume=r.volume or 0) for r in rows]
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch candle history for {pair}")
...
async def get_trading_pair_details(
    ...
    try:
        ...
        if not pair_info:
            raise HTTPException(status_code=404, detail=f"Trading pair {pair} not found")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch details for {pair}")
...
async def get_advanced_market_analysis(
    ...
    try:
        ...
        analysis_result = market_analysis_service.analyze(ohlcv_data, volatility_analyzer)
        ...
        for indicator in indicators:
            if indicator == "rsi" and len(closes) >= period:
                ...
                signal = "bullish" if rsi_value < 30 else "bearish" if rsi_value > 70 else "neutral"
                ...
                macd = ema12 - ema26
                signal_line = sum([ema12 - ema26 for _ in range(min(9, len(closes)))]) / min(9, len(closes))
                ...
                calculated_indicators.append(TechnicalIndicator(
                    name="bollinger",
                    value=round((current_price - sma) / (std * 2) if current_price and std != 0 else 0, 2),
                    signal=signal,
                    period=20
                ))
        ...
        trend = "bullish" if trend_strength > 0.6 else "bearish" if trend_strength < 0.4 else "sideways"
            ...
            try:
                ...
                if current_price:
                    update = RealTimeMarketData(
                        ...
                        timestamp=int(__import__('time').time() * 1000),
                        bid_price=current_order_book.bids[0][0] if current_order_book.bids else None,
                        ask_price=current_order_book.asks[0][0] if current_order_book.asks else None
                    )
