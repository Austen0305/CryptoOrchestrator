...
async def generate_license(
    ...
    try:
        ...
        if (
            ...
            raise HTTPException(
                status_code=403, detail="Not authorized to generate licenses"
            )
...
async def validate_license(
    ...
    try:
        status = license_service.validate_license_key(
            license_key=request.license_key, machine_id=request.machine_id
        )
        ...
        return {
            ...
            "activated_at": (
                status.activated_at.isoformat() if status.activated_at else None
            ),
...
async def activate_license(
    ...
    try:
        ...
        if not status.valid:
            raise HTTPException(
                status_code=400, detail=status.message or "Invalid license"
            )
        ...
        bound = license_service.bind_license_to_machine(
            license_key=request.license_key, machine_id=machine_id
        )

        if not bound:
            raise HTTPException(
                status_code=500, detail="Failed to bind license to machine"
            )
