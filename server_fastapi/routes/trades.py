...
class TradeCreate(BaseModel):
    ...
    type: Optional[str] = "market"  # 'market', 'limit', 'stop', 'stop-limit', 'take-profit', 'trailing-stop'
    amount: float
...
async def get_trades(
    ...
    try:
        ...
        for trade in db_trades:
            ...
            timestamp_str = trade.executed_at.isoformat() if trade.executed_at else trade.timestamp.isoformat()
...
async def create_trade(
    ...
    try:
        ...
        if trade.mode == "real":
            ...
            requires_kyc = await kyc_service.require_kyc_for_trading(user_id, trade_value)
            if requires_kyc:
        ...
        if trade.mode == "real" and two_factor_service.require_2fa_for_trading(user_id):
            if not trade.mfa_token:
                raise HTTPException(
                    status_code=400,
                    detail="2FA token required for real money trades"
                )
        ...
        if trade.mode == "real" or trade.mode == "live":
            try:
                from ..services.wallet_trading_integration import WalletTradingIntegration
                from ..database import get_db_session
                ...
                if trade_value <= 0:
                    ...
                    trade_value = trade.amount * 50000  # Estimate with current market price
                ...
                raise HTTPException(
                    status_code=500,
                    detail="Failed to reserve funds for trade"
                )
            ...
            try:
                ...
                if db_trade:
                    ...
                    if trade.side == "sell" and db_trade.status == "completed":
                        ...
                        execution_price = order_result.get("price") or trade.price or db_trade.price
                        ...
                        buy_trades_query = select(Trade).where(
                            and_(
                        ...
                        for buy_trade in buy_trades:
                            ...
                            if buy_trade.amount <= sell_amount:
                                ...
                                cost_basis = buy_trade.amount * buy_trade.price + (buy_trade.fee or 0.0)
                                total_cost_basis += cost_basis
                                sell_amount -= buy_trade.amount
                            else:
                                # Use partial buy trade
                                cost_basis = sell_amount * buy_trade.price + (sell_amount / buy_trade.amount) * (buy_trade.fee or 0.0)
                                total_cost_basis += cost_basis
                                sell_amount = 0
                        ...
                        pnl_percent = (realized_pnl / total_cost_basis * 100) if total_cost_basis > 0 else 0.0
                ...
                logger.info(f"Real money trade executed: trade_id={trade_id}, order_id={order_result.get('order_id')}")
        ...
        if db_trade:
            ...
            timestamp_str = db_trade.executed_at.isoformat() if db_trade.executed_at else db_trade.timestamp.isoformat()
