...
async def get_orderbook_snapshot(
    ...
    # Check rate limit
    allowed, error_msg = enterprise_api_tier_service.check_rate_limit(
        user_id, "orderbook_snapshot"
    )
    if not allowed:
...
async def get_orderbook_deltas(
    pair: str,
    current_user: Annotated[dict, Depends(get_current_user)],
    since_sequence: int | None = Query(
        None, description="Get deltas after this sequence"
    ),
    ...
    try:
        ...
        return [
            OrderBookDeltaResponse(
                ...
                bid_updates=[
                    [float(price), float(qty)] for price, qty in delta.bid_updates
                ],
                ask_updates=[
                    [float(price), float(qty)] for price, qty in delta.ask_updates
                ],
            )
            for delta in deltas
        ]
    ...
    try:
        ...
        while True:
            try:
                ...
                # Send heartbeat
                await websocket.send_json(
                    {"type": "heartbeat", "timestamp": time.time_ns()}
                )
