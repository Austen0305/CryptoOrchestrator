...
@router.get("/recent")
@cache_query_result(ttl=30, key_prefix="activity_recent", include_user=True, include_params=True)
        ...
        async with get_db_context() as db:
            ...
            for bot in bots:
                activities.append(ActivityItem(
                    ...
                    timestamp=bot.updated_at.isoformat() if hasattr(bot.updated_at, 'isoformat') else str(bot.updated_at),
            ...
            for alert in alerts:
                activities.append(ActivityItem(
                    ...
                    status="warning" if alert.severity == "medium" else "error" if alert.severity == "high" else "success"
                ))
