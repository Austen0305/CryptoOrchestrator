...
class BacktestEngine:
    ...
    async def run_monte_carlo(self, config: MonteCarloConfig) -> MonteCarloResult:
        ...
        for i in range(config.num_simulations):
            ...
            metrics = self._calculate_metrics(
                simulated_trades,
                config.backtest_config.strategy.initial_capital
            )
        ...
        simulation_id = f"mc_{config.backtest_config.symbol}_{int(datetime.now().timestamp())}"
        ...
        return MonteCarloResult(
            ...
            base_metrics=self._metrics_to_dict(
                self._calculate_metrics(base_trades, config.backtest_config.strategy.initial_capital)
            ),
    ...
    async def run_walk_forward(self, config: WalkForwardConfig) -> WalkForwardResult:
        ...
        while current_start < end_date:
            ...
            optimal_params.append({
                "period": current_start.isoformat(),
        ...
        analysis_id = f"wf_{config.backtest_config.symbol}_{int(datetime.now().timestamp())}"
        ...
        for signal in signals:
            ...
            if signal["signal"] == "buy" and position is None:
                position = {
                    ...
                    "size": config.strategy.position_size_pct * config.strategy.initial_capital / price
                }
            
            elif signal["signal"] == "sell" and position is not None:
                exit_price = price - slippage
                pnl = (exit_price - position["entry_price"]) * position["size"] - 2 * commission
    ...
    def _calculate_metrics(self, trades: List[Trade], initial_capital: float) -> PerformanceMetrics:
        ...
        # Calmar ratio
        annualized_return = total_return_pct * (252 / len(trades)) if len(trades) > 0 else 0
    ...
    def _build_equity_curve(self, trades: List[Trade], initial_capital: float) -> List[Dict]:
    ...
    def _empty_metrics(self) -> PerformanceMetrics:
        ...
        return PerformanceMetrics(
            ...
            profit_factor=0, avg_win=0, avg_loss=0, avg_trade=0, num_trades=0,
            num_winning=0, num_losing=0, avg_holding_period_hours=0,
