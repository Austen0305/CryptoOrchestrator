...
@router.get("/advanced")
@cache_query_result(ttl=120, key_prefix="advanced_metrics", include_user=True, include_params=True)
async def get_advanced_metrics(
    ...
    try:
        ...
        async with get_db_context() as db:
            ...
            gross_loss = abs(sum(losing_profits)) if losing_profits else 0.001  # Avoid division by zero
...
@cache_query_result(ttl=300, key_prefix="daily_pnl", include_user=True, include_params=True)
...
@router.get("/drawdown")
@cache_query_result(ttl=300, key_prefix="drawdown_chart", include_user=True, include_params=True)
