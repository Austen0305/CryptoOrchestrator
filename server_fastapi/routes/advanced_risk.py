...
class PositionSizingRequest(BaseModel):
    ...
    method: str = Field(..., description="Position sizing method: fixed_fractional, kelly_criterion, volatility_based")
    account_balance: float = Field(..., gt=0, description="Current account balance")
    risk_per_trade: float = Field(..., gt=0, le=1, description="Risk per trade as decimal (e.g., 0.02 for 2%)")
    entry_price: float = Field(..., gt=0, description="Planned entry price")
    stop_loss: float = Field(..., gt=0, description="Stop loss price")
    win_rate: Optional[float] = Field(None, ge=0, le=1, description="Historical win rate (for Kelly)")
    avg_win: Optional[float] = Field(None, description="Average win amount (for Kelly)")
    avg_loss: Optional[float] = Field(None, description="Average loss amount (for Kelly)")
    volatility: Optional[float] = Field(None, description="Asset volatility (for volatility-based)")
...
class VaRRequest(BaseModel):
    ...
    confidence_level: float = Field(0.95, ge=0.9, le=0.99, description="Confidence level (0.95 or 0.99)")
...
class RiskLimitsRequest(BaseModel):
    ...
    max_position_size: Optional[float] = Field(0.20, description="Max position size as % of portfolio")
...
class StopLossRequest(BaseModel):
    ...
    max_loss_pct: float = Field(0.02, gt=0, le=0.1, description="Max loss % (default 2%)")
...
async def calculate_position_size(request: PositionSizingRequest):
    ...
    **Methods:**
    ...
    try:
        ...
        risk_percentage = (risk_amount / request.account_balance) * 100 if request.account_balance > 0 else 0
...
async def calculate_take_profit(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error calculating take profit: {str(e)}")
