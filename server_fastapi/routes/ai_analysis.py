...
async def get_bot_ai_analysis(bot_id: str):
    ...
    try:
        ...
        overall_score = _calculate_overall_score(insights, market_sentiment, risk_assessment)
...
async def _generate_recommendations(bot: Dict, insights: List[TradeInsight]) -> List[str]:
    ...
    if bot["win_rate"] > 0.6:
        recommendations.append("Consider gradually increasing position sizes to scale profitability.")
    
    recommendations.append("Review and adjust stop-loss levels based on recent volatility.")
    recommendations.append("Monitor key support/resistance levels for optimal entry timing.")
    
    return recommendations


def _calculate_overall_score(insights: List[TradeInsight], sentiment: Dict, risk: Dict) -> float:
    """Calculate overall bot performance score (0-100)"""
    # Weighted scoring
    insight_score = sum(i.impact_score for i in insights) / len(insights) if insights else 0
    sentiment_score = sentiment["confidence"] * 10 if sentiment["sentiment"] == "bullish" else -sentiment["confidence"] * 5
